...
**Why it exists:** Provides a canonical IEEE SystemVerilog object model that Surelog populates during elaboration. Tools like Yosys, Verilator, linters, and formal tools consume the UHDM graph via VPI or C++ API.
...
- Python bindings (pybind11 → `pyuhdm`)
...
| `include/uhdm/` | Generated headers + hand-written foundations (`any.h`, `RTTI.h`, `vpi_user.h`) |
...
model/*.yaml → scripts/loader.py (parse + validate) → scripts/classes.py + 28 other generators
             → include/uhdm/*.h + src/*.cpp + python/py_*.cpp
```
...
s.save("design.uhdm");          // Cap'n Proto binary save
s.restore("design.uhdm");       // reload
```
...
- Python: `pytest tests/ -v` (requires `pyuhdm` installed: `pip install -e .`)
