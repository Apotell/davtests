...
constexpr std::array<T, LL + RL> join(const std::array<T, LL> rhs, const std::array<T, RL> lhs, num_tuple<LLs...>,
                                      num_tuple<RLs...>) {
...
inline constexpr uint32_t RTTIHash(const char *const str, uint32_t hash) noexcept {
  return (*str == '\0') ? hash : RTTIHash(str + 1, (hash ^ static_cast<uint32_t>(*str)) * kFNV1aPrime32);
}
}  // namespace internal
...
class RTTI {
 ...
 protected:
  ...
  inline void *AsOfTypeRecurse(typeid_t tid) { return (tid == RTTI::kTypeId) ? static_cast<void *>(this) : nullptr; }
  ...
  inline bool IsOfType(typeid_t tid) const {
    ...
    for (size_t i = 0, j = count - 1; i <= j; ++i, --j) {
      if ((typeIds[i] == tid) || (typeIds[j] == tid)) return true;
    }
 ...
 public:
  template <typename I, typename T = typename std::remove_pointer<I>::type,
...
inline RTTI::typeid_t RTTI::GetTypeId() const { return RTTI::kTypeId; }
...
inline void *RTTI::AsOfType(typeid_t tid) { return AsOfTypeRecurse(tid); }

inline const void *RTTI::AsOfType(typeid_t tid) const { return AsOfTypeRecurse(tid); }
...
#define UHDM_IMPLEMENT_RTTI(classType, baseType)                                                                \
 public:                                                                                                        \
  ...
  static constexpr auto kTypeIds =                                                                              \
      uhdm::internal::join(std::array<typeid_t, 1>{thistype_t::kTypeId}, basetype_t::kTypeIds);                 \
                                                                                                                \
 protected:                                                                                                     \
  ...
  inline virtual const RTTI::typeid_t *GetTypeIds(size_t &count) const override {                               \
    count = thistype_t::kTypeIds.size();                                                                        \
    return thistype_t::kTypeIds.data();                                                                         \
  }                                                                                                             \
...
#define UHDM_IMPLEMENT_RTTI_2_BASES(classType, baseType1, baseType2)                                            \
 public:                                                                                                        \
  ...
  static constexpr typeid_t kTypeId =                                                                           \
      uhdm::internal::RTTIHash("/" #classType, base1type_t::kTypeId ^ base2type_t::kTypeId);                    \
  static constexpr auto kTypeIds =                                                                              \
      uhdm::internal::join(std::array<typeid_t, 1>{thistype_t::kTypeId},                                        \
...
#define UHDM_IMPLEMENT_RTTI_CAST_FUNCTIONS(fname, baseType)                                                          \
  ...
  inline std::shared_ptr<const T> fname(std::shared_ptr<const baseType> const &u) noexcept {                         \
    return (u && (u->template Cast<const T>() != nullptr)) ? std::static_pointer_cast<const T>(u)                    \
                                                           : std::shared_ptr<const T>(nullptr);                      \
  }
...
#define UHDM_IMPLEMENT_RTTI_VIRTUAL_CAST_FUNCTIONS(fname, baseType)                                      \
  ...
  template <typename T, typename = typename std::enable_if<std::is_base_of<baseType, T>::value>::type>   \
  inline std::shared_ptr<T> fname(std::shared_ptr<baseType> const &u) noexcept {                         \
    return (u && (u->template VirtualCast<T>() != nullptr)) ? std::static_pointer_cast<T>(u)             \
                                                            : std::shared_ptr<T>(nullptr);               \
  }                                                                                                      \
  template <typename T, typename = typename std::enable_if<std::is_base_of<baseType, T>::value>::type>   \
  inline std::shared_ptr<const T> fname(std::shared_ptr<const baseType> const &u) noexcept {             \
    return (u && (u->template VirtualCast<const T>() != nullptr)) ? std::static_pointer_cast<const T>(u) \
                                                                  : std::shared_ptr<const T>(nullptr);   \
  }
