...
class Elaborator final : public VpiListener, public Cloner {
 ...
 private:
  ...
  // clang-format off
  using Cloner::copy;
  void copy(const Begin* source, Begin* target) override;
  void copy(const BitSelect* source, BitSelect* target) override;
  void copy(const ClassDefn* source, ClassDefn* target) override;
  void copy(const ClassTypespec* source, ClassTypespec* target) override;
  void copy(const ClockingBlock* source, ClockingBlock* target) override;
  void copy(const Design* source, Design* target) override;
  void copy(const Disable* source, Disable* target) override;
  void copy(const ForeachStmt* source, ForeachStmt* target) override;
  void copy(const ForkStmt* source, ForkStmt* target) override;
  void copy(const FuncCall* source, FuncCall* target) override;
  void copy(const Function* source, Function* target) override;
  void copy(const GenScope* source, GenScope* target) override;
  void copy(const IndexedPartSelect* source, IndexedPartSelect* target) override;
  void copy(const Instance* source, Instance* target) override;
  void copy(const IntTypespec* source, IntTypespec* target) override;
  void copy(const MethodFuncCall* source, MethodFuncCall* target) override;
  void copy(const MethodTaskCall* source, MethodTaskCall* target) override;
  void copy(const Modport* source, Modport* target) override;
  void copy(const Package* source, Package* target) override;
  void copy(const PartSelect* source, PartSelect* target) override;
  void copy(const Ports* source, Ports* target) override;
  void copy(const Process* source, Process* target) override;
  void copy(const RefObj* source, RefObj* target) override;
  void copy(const RefTypespec* source, RefTypespec* target) override;
  void copy(const Task* source, Task* target) override;
  void copy(const TaskCall* source, TaskCall* target) override;
  void copy(const TaskFunc* source, TaskFunc* target) override;
  void copy(const Tchk* source, Tchk* target) override;
  void copy(const Typespec* source, Typespec* target) override;
  void copy(const Udp* source, Udp* target) override;
  void copy(const VarSelect* source, VarSelect* target) override;
  // clang-format on
 ...
 private:
  ...
  void enterGenScope(const GenScope *object, vpiHandle handle);
  void leaveGenScope(const GenScope *object, vpiHandle handle);

  using ComponentMap = std::map<std::string, const Any*, std::less<>>;
  // Instance context stack
  using InstStack = std::vector<std::tuple<const Any*, ComponentMap, ComponentMap, ComponentMap, ComponentMap>>;
