...
struct PathIdHasher final {
  inline size_t operator()(const PathId &value) const {
    return std::hash<const SymbolTable *>()(value.getSymbolTable()) ^ std::hash<RawPathId>()((RawPathId)value);
  }
};

struct PathIdLessThanComparer final {
  inline bool operator()(const PathId &lhs, const PathId &rhs) const {
    return (lhs.getSymbolTable() == rhs.getSymbolTable())
               ? ((RawPathId)lhs < (RawPathId)rhs)
               : std::less<const SymbolTable *>()(lhs.getSymbolTable(), rhs.getSymbolTable());
  }
};
...
template<typename T>
using PathIdMap = std::map<PathId, T, PathIdLessThanComparer>;
