...
// correctly classify each instantiation and produce the right UHDM objects.
...
TEST_F(Instantiations, ModuleDefinitions) {
  ASSERT_NE(uhdm::findByName<uhdm::Module>("work@passthru", m_design->getAllModules()), nullptr);
  ASSERT_NE(uhdm::findByName<uhdm::Module>("work@adder",    m_design->getAllModules()), nullptr);
  ASSERT_NE(uhdm::findByName<uhdm::Module>("work@top",      m_design->getAllModules()), nullptr);
  ASSERT_NE(uhdm::findByName<uhdm::Module>("work@gen_top",  m_design->getAllModules()), nullptr);
}

TEST_F(Instantiations, InterfaceDefinitions) {
  ASSERT_NE(uhdm::findByName<uhdm::Interface>("work@simple_if", m_design->getAllInterfaces()), nullptr);
  ASSERT_NE(uhdm::findByName<uhdm::Interface>("work@bus_if",    m_design->getAllInterfaces()), nullptr);
}
...
TEST_F(Instantiations, UdpDefinition) {
  ASSERT_NE(uhdm::findByName<uhdm::UdpDefn>("work@my_udp", m_design->getAllUdps()), nullptr);
}

// ---------------------------------------------------------------------------
// Helper: return the RefInstance named 'instName' inside 'parent', or null.
// ---------------------------------------------------------------------------
static const uhdm::RefInstance *findRefInst(std::string_view instName,
                                            const uhdm::Module *parent) {
  return uhdm::findByName<uhdm::RefInstance>(instName, parent->getRefInstances());
}
...
TEST_F(Instantiations, InterfaceInstantiation) {
  ...
  const uhdm::RefInstance *const u_simple_if = findRefInst("u_simple_if", top);
  ...
  EXPECT_NE(u_simple_if->getTypespec()->getActual<uhdm::InterfaceTypespec>(),
            nullptr) << "u_simple_if typespec is not InterfaceTypespec";

  // Parameterized interface instantiation — isInterfaceElem("bus_if")
  const uhdm::RefInstance *const u_bus = findRefInst("u_bus", top);
  ASSERT_NE(u_bus, nullptr) << "u_bus RefInstance not found";
  ASSERT_NE(u_bus->getTypespec(), nullptr) << "u_bus has no typespec";
  EXPECT_NE(u_bus->getTypespec()->getActual<uhdm::InterfaceTypespec>(),
            nullptr) << "u_bus typespec is not InterfaceTypespec";
}
...
TEST_F(Instantiations, ModuleInstantiation) {
  ...
  const uhdm::RefInstance *const u_pt = findRefInst("u_pt", top);
  ASSERT_NE(u_pt, nullptr) << "u_pt RefInstance not found";
  ASSERT_NE(u_pt->getTypespec(), nullptr) << "u_pt has no typespec";
  EXPECT_NE(u_pt->getTypespec()->getActual<uhdm::ModuleTypespec>(),
            nullptr) << "u_pt typespec is not ModuleTypespec";

  // Parameterized module instantiation — isModuleElem("adder")
  const uhdm::RefInstance *const u_add = findRefInst("u_add", top);
  ASSERT_NE(u_add, nullptr) << "u_add RefInstance not found";
  ASSERT_NE(u_add->getTypespec(), nullptr) << "u_add has no typespec";
  EXPECT_NE(u_add->getTypespec()->getActual<uhdm::ModuleTypespec>(),
            nullptr) << "u_add typespec is not ModuleTypespec";
}
...
TEST_F(Instantiations, ProgramInstantiation) {
  ...
  const uhdm::RefInstance *const u_prog = findRefInst("u_prog", top);
  ASSERT_NE(u_prog, nullptr) << "u_prog RefInstance not found";
  ASSERT_NE(u_prog->getTypespec(), nullptr) << "u_prog has no typespec";
  EXPECT_NE(u_prog->getTypespec()->getActual<uhdm::ProgramTypespec>(),
            nullptr) << "u_prog typespec is not ProgramTypespec";
}
...
TEST_F(Instantiations, UnsupportedInstantiation) {
  ...
  const uhdm::RefInstance *const u_bb = findRefInst("u_bb", top);
  ASSERT_NE(u_bb, nullptr) << "u_bb RefInstance not found";
  ASSERT_NE(u_bb->getTypespec(), nullptr) << "u_bb has no typespec";
  EXPECT_NE(u_bb->getTypespec()->getActual<uhdm::UnsupportedTypespec>(),
            nullptr) << "u_bb typespec is not UnsupportedTypespec";
}
...
TEST_F(Instantiations, MacroDefinedModuleInstantiation) {
  ...
  const uhdm::RefInstance *const u_macro_mod = findRefInst("u_macro_mod", top);
  ...
  EXPECT_NE(u_macro_mod->getTypespec()->getActual<uhdm::ModuleTypespec>(),
            nullptr) << "u_macro_mod typespec is not ModuleTypespec — "
...
TEST_F(Instantiations, UdpInstantiation) {
  ...
  const uhdm::Udp *u_udp = nullptr;
  for (const uhdm::Primitive *const prim : *top->getPrimitives()) {
    if (const uhdm::Udp *const udp = any_cast<const uhdm::Udp *>(prim)) {
...
TEST_F(Instantiations, GenerateBlockInstantiations) {
  const uhdm::Module *const gen_top =
      uhdm::findByName<uhdm::Module>("work@gen_top", m_design->getAllModules());
  ...
  const uhdm::GenRegion *gen_region = nullptr;
  for (const uhdm::Any *const stmt : *gen_top->getGenStmts()) {
    gen_region = any_cast<const uhdm::GenRegion *>(stmt);
  ...
  const uhdm::GenFor *const gen_for = gen_region->getStmt<uhdm::GenFor>();
  ASSERT_NE(gen_for, nullptr) << "GenRegion stmt is not a GenFor";

  // The named begin block (gen_lane) is the stmt of the GenFor
  const uhdm::Begin *const body = gen_for->getStmt<uhdm::Begin>();
  ...
  for (const uhdm::Any *const s : *body->getStmts()) {
    const uhdm::RefInstance *const ri = any_cast<const uhdm::RefInstance *>(s);
    if (ri == nullptr || ri->getTypespec() == nullptr) continue;
    if (ri->getTypespec()->getActual<uhdm::InterfaceTypespec>() != nullptr) found_if  = true;
    if (ri->getTypespec()->getActual<uhdm::ModuleTypespec>()    != nullptr) found_mod = true;
  }
