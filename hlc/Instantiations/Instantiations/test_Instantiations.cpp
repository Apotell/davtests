...
// ---------------------------------------------------------------------------
static const hldb::RefInstance *findRefInst(std::string_view instName,
                                            const hldb::Module *parent) {
...
TEST_F(Instantiations, InterfaceInstantiation) {
  ...
  EXPECT_NE(u_simple_if->getTypespec()->getActual<hldb::InterfaceTypespec>(),
  ...
  EXPECT_NE(u_bus->getTypespec()->getActual<hldb::InterfaceTypespec>(),
...
TEST_F(Instantiations, ModuleInstantiation) {
  ...
  EXPECT_NE(u_pt->getTypespec()->getActual<hldb::ModuleTypespec>(),
  ...
  EXPECT_NE(u_add->getTypespec()->getActual<hldb::ModuleTypespec>(),
...
TEST_F(Instantiations, ProgramInstantiation) {
  ...
  EXPECT_NE(u_prog->getTypespec()->getActual<hldb::ProgramTypespec>(),
...
TEST_F(Instantiations, UnsupportedInstantiation) {
  ...
  EXPECT_NE(u_bb->getTypespec()->getActual<hldb::UnsupportedTypespec>(),
...
TEST_F(Instantiations, MacroDefinedModuleInstantiation) {
  ...
  EXPECT_NE(u_macro_mod->getTypespec()->getActual<hldb::ModuleTypespec>(),
...
TEST_F(Instantiations, GenerateBlockInstantiations) {
  const hldb::Module *const gen_top =
      hldb::findByName<hldb::Module>("work@gen_top", m_design->getAllModules());
