...
// Tests for the IEEE 1800-2023 Clause 21 (input/output system tasks and
// system functions) error scenarios catalogued in docs/error_catalog.xml
// (rows 754, 762, 765, 779, 780, 781, 782, 786).
//
...
//
// Fixture: 21--error_rules.sv, compiled by 21--error_rules.hlc. All eight
...
// there are no _inv* sibling files for this chapter, even though the
// colleague's DUT source labels three of them (rows 780-782, all
// $dumpports scenarios) Category PARSE: each is a well-formed system-task
// call syntactically (a system task call takes an ordinary expression
// list), so the actual, reportable fault in every case is the kind of
// argument supplied, not the surrounding grammar -- matching the DUT
// author's own "Code Notes" for each of these three rows.
//
// None of the ErrorDefinition codes this chapter's rows name
// (COMP_FORMAT_ARGUMENT_MISMATCH, COMP_VALUE_OUT_OF_RANGE,
// COMP_ILLEGAL_EXPRESSION_CONTEXT, COMP_MISSING_ARGUMENT) has a single call
// site anywhere in src/ (confirmed by grep) -- every one of these 8 rows is
// unimplemented today. Every TEST_F below therefore asserts the diagnostic
// the standard requires and is expected to fail (red) until Clause 21
// semantic checking is implemented -- per project convention this is
// written as the real assertion, never as an assertion that the diagnostic
// is absent.
...
// --- row 754: every % format specifier needs a corresponding argument (21.2.1) ---

TEST_F(Chapter21ErrorRulesTest, Row754_DisplayFormatSpecifierNeedsCorrespondingArgument) {
  // catalog row 754 | 21.2.1 | COMP
  GTEST_SKIP() << "not yet implemented in HLC's Linter (IEEE 1800-2023 21.2.1)";
  ...
  // argument shall be supplied after the string literal. r754_m calls
  // "$display("a=%d b=%d", a);" -- the second %d has no matching argument.
  EXPECT_NE(findError(ErrorDefinition::COMP_FORMAT_ARGUMENT_MISMATCH, "r754_m"), nullptr)
      << "every %-specifier in a $display format string needs a corresponding argument (IEEE 1800-2023 21.2.1)";
}
