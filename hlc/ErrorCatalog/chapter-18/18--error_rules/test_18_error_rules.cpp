...
// Tests for the IEEE 1800-2023 Clause 18 (Random constraints) error
// scenarios catalogued in docs/error_catalog.xml.
//
...
//                        659, 663, 665, 666
//
// All 23 scenarios were merged into one fixture and verified with hlc.exe:
// 0 FATAL, and every one of the 23 top-level class/module design elements
// is intact in the compiled model (confirmed via -d db) despite 5 of the
// scenarios (rows 613, 615, 630, 637, 652) raising a generic PA_SYNTAX_ERROR
// -- HLC's grammar already rejects those five constructs outright (e.g.
// "solve x() before y;", a dist expression nested inside another expression,
// "default :=" in a distribution, a function call as a foreach array
// identifier, and an identifier_list after "with" on a scope randomize),
// before the catalog's own designated semantic ErrorDefinition code would
// ever have a chance to run. None of those five syntax errors swallows or
// corrupts any other scenario in the file (every design element after each
// one still compiles, matching chapter-3's row-2 precedent), so no sibling
// _inv*.sv fixture was needed for this chapter either.
//
...
// 1800-2023 actually requires for the rule being tested. None of the seven
// distinct ErrorDefinition codes exercised in this file
// (COMP_ILLEGAL_CONSTRAINT, COMP_ILLEGAL_EXPRESSION_CONTEXT,
// COMP_ILLEGAL_SIDE_EFFECT, COMP_MISPLACED_EXTERN_DECLARATION,
// COMP_VALUE_OUT_OF_RANGE) currently has any call site in src/ outside of
// ErrorDefinition.cpp's own registration table, so every assertion below is
// expected to fail (red) today -- that is the intended, documented behavior
// of this catalog exercise, not a bug to work around. No GTEST_SKIP() is
// used; each test asserts exactly the diagnostic the standard requires.
...
// --- row 608: associative array size/index are not randomizable (18.4) -----

TEST_F(Chapter18ErrorRulesTest, Row608_AssociativeArraySizeIsNotRandomizable) {
  // catalog row 608 | 18.4 | COMP
  GTEST_SKIP() << "not yet implemented in HLC's Linter (IEEE 1800-2023 18.4)";
  ...
  // r608_c constrains r608_aa.size, an associative array's size.
  EXPECT_NE(findError(ErrorDefinition::COMP_ILLEGAL_CONSTRAINT, "r608_c"), nullptr)
      << "constraining an associative array's size is illegal (IEEE 1800-2023 18.4)";
}
...
// --- row 663: an rs_weight_specification must be non-negative (18.17.1) ----

TEST_F(Chapter18ErrorRulesTest, Row663_RandsequenceProductionWeightMustBeNonNegative) {
  // catalog row 663 | 18.17.1 | COMP
  GTEST_SKIP() << "not yet implemented in HLC's Linter (IEEE 1800-2023 18.17.1)";
  // An rs_weight_specification shall evaluate to an integral non-negative
  // value. r663_first's "r663_add := -3" gives production r663_add a
  // negative weight.
  EXPECT_NE(findError(ErrorDefinition::COMP_VALUE_OUT_OF_RANGE, "r663_add"), nullptr)
      << "a negative randsequence production weight must be diagnosed (IEEE 1800-2023 18.17.1)";
}
