...
namespace hlc {
class PreprocPersist2Test : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "t_preproc_persist2.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

// The file contains only preprocessor directives; no module is declared.
TEST_F(PreprocPersist2Test, NoModules) {
  ASSERT_TRUE((m_design->getAllModules() == nullptr) || (m_design->getAllModules()->size() == 0u))
      << "t_preproc_persist2 contains no module declarations";
}

// LRM 22.4: `include must cause the compiler to record the included file.
TEST_F(PreprocPersist2Test, IncludedFileRecorded) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf =
      hldb::findByName<hldb::SourceFile>("t_preproc_persist2.v", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  ASSERT_NE(sf->getIncludes(), nullptr) << "t_preproc_persist2.v must record the included file t_preproc_persist_inc.v";
...
TEST_F(PreprocPersist2Test, CommonGuardDefinedInInclude) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf =
      hldb::findByName<hldb::SourceFile>("t_preproc_persist2.v", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
...
TEST_F(PreprocPersist2Test, CommonGuardHasNoArguments) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf =
      hldb::findByName<hldb::SourceFile>("t_preproc_persist2.v", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
...
TEST_F(PreprocPersist2Test, CommonGuardHasTokens) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf =
      hldb::findByName<hldb::SourceFile>("t_preproc_persist2.v", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
