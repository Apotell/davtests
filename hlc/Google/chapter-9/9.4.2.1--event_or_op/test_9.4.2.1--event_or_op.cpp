...
// Source under test: tests/Google/chapter-9/9.4.2.1--event_or_op.sv
//
//   module block_tb ();
//   	wire a = 0;
//   	wire b = 0;
...
//   endmodule
//
// IEEE 1800-2023 clause tested: 9.4.2.1 "Event OR operator". The 'or'
// keyword joins event_expressions in an event_control list; the compiler
// triggers whenever any one of the joined expressions triggers. Per the
// same clause the ',' operator (see 9.4.2.1--event_comma_op.sv) has
// identical meaning, so this file is expected to elaborate to the same
// EventControl condition shape as the comma-separated version (each
// verified independently, since CMake builds each test_*.cpp into its own
// executable).
//
// Checked:
...
//     (IEEE 1800-2023 6.7.1); the exact internal representation is
...
namespace hlc {
class EventOrOpTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "9.4.2.1--event_or_op.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

TEST_F(EventOrOpTest, ModuleExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
}

TEST_F(EventOrOpTest, ABCDAreNetsOutIsVariable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(EventOrOpTest, AInitializerIsPreserved) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(EventOrOpTest, ExactlyOnePlainAlwaysProcess) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(EventOrOpTest, OrSeparatedEventListIsEventOrOfFourOperands) {
  ...
  ASSERT_NE(eventControl, nullptr) << "always @(a or b or c or d) must produce an EventControl";
...
TEST_F(EventOrOpTest, ControlledStatementIsOutAssignment) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
