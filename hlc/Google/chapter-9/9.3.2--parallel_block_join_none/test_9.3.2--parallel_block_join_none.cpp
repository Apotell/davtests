...
// Tests for 9.3.2--parallel_block_join_none.sv (tags: 9.3.2)
//   module parallel_tb ();
//     reg a = 0;
//     reg b = 0;
//     reg c = 0;
...
//   endmodule
//
// IEEE 1800-2017 Sec 9.3.2 "Parallel blocks (fork-join)": "join_none"
// (unlike 9.3.2--parallel_block_join.sv's "join" and
// 9.3.2--parallel_block_join_any.sv's "join_any") means the parent
// process continues immediately WITHOUT waiting for any of the fork's
// statements to complete -- the forked statements become fully detached
// child processes. Same ForkStmt (Scope) node structure as its siblings,
// distinguished purely by getJoinType() (vpiJoinNone here). "fork...
// join_none" is itself the single statement following "initial" (no
// begin/end needed), so it sits directly on the Initial's stmt.
//
// "reg a/b/c = 0" -- same reasoning as the other 9.3.2 files: each a
...
//   - the ForkStmt's getJoinType() is vpiJoinNone; it has exactly 3
//     statements
...
class ParallelBlockJoinNoneTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "9.3.2--parallel_block_join_none.hlc"}); }
...
TEST_F(ParallelBlockJoinNoneTest, ModuleExists) { EXPECT_NE(getModule(), nullptr); }

TEST_F(ParallelBlockJoinNoneTest, ModuleHasThreeVariables) {
  const hldb::Module *const mod = getModule();
...
TEST_F(ParallelBlockJoinNoneTest, VariableAExists) { EXPECT_NE(getVariable("a"), nullptr); }
TEST_F(ParallelBlockJoinNoneTest, VariableBExists) { EXPECT_NE(getVariable("b"), nullptr); }
TEST_F(ParallelBlockJoinNoneTest, VariableCExists) { EXPECT_NE(getVariable("c"), nullptr); }

// "reg a/b/c" have no net-type keyword, so per IEEE 1800-2023 Sec 6.7/6.8
// none must also appear in the module's net collection.
TEST_F(ParallelBlockJoinNoneTest, VariablesAreNotDuplicatedAsNets) {
  const hldb::Module *const mod = getModule();
  ASSERT_NE(mod, nullptr);
...
TEST_F(ParallelBlockJoinNoneTest, VariableTypespecsAreLogicUnsignedWithNoRanges) {
...
TEST_F(ParallelBlockJoinNoneTest, AllVariablesInitialValueIsConstantZero) {
  for (std::string_view name : {"a", "b", "c"}) {
    const hldb::Variable *const v = getVariable(name);
    ASSERT_NE(v, nullptr);
...
TEST_F(ParallelBlockJoinNoneTest, ModuleHasOneProcess) {
  const hldb::Module *const mod = getModule();
  ASSERT_NE(mod, nullptr);
...
TEST_F(ParallelBlockJoinNoneTest, TheOneProcessIsInitial) { EXPECT_NE(getInitialProcess(), nullptr); }

// 9.3.2: "fork...join_none" is itself the single statement following
// "initial" -- no enclosing Begin is needed or expected.
TEST_F(ParallelBlockJoinNoneTest, InitialStmtIsDirectlyAForkStmt) {
  EXPECT_NE(getForkStmt(), nullptr) << "9.3.2: 'fork...join_none' must be modeled as a ForkStmt";
}

TEST_F(ParallelBlockJoinNoneTest, ForkJoinTypeIsJoinNone) {
  const hldb::ForkStmt *const fork = getForkStmt();
  ASSERT_NE(fork, nullptr);
  EXPECT_EQ(fork->getJoinType(), vpiJoinNone) << "9.3.2: 'join_none' must have JoinType vpiJoinNone";
}

TEST_F(ParallelBlockJoinNoneTest, ForkHasThreeStmts) {
  const hldb::ForkStmt *const fork = getForkStmt();
  ASSERT_NE(fork, nullptr);
...
TEST_F(ParallelBlockJoinNoneTest, FirstStmtIsBlockingAssignToA) {
...
TEST_F(ParallelBlockJoinNoneTest, SecondStmtIsBlockingAssignToB) {
...
TEST_F(ParallelBlockJoinNoneTest, ThirdStmtIsBlockingAssignToC) {
...
TEST_F(ParallelBlockJoinNoneTest, ReferencesAreNotFailedBinds) {
