...
// Source under test: tests/Google/chapter-9/9.4.2--event_control_negedge.sv
//
//   module block_tb ();
//   	reg [3:0] a = 0;
//   	wire clk = 0;
...
// posedge | negedge | edge. "negedge" restricts the trigger to a 1/x/z -> 0
// transition of its operand. The VPI mapping (negedge event_expression ->
// an operation with vpiNegedgeOp and a single operand) is part of the
// standard's own Annex, so it is asserted directly.
//
// Checked:
...
//     an Operation with opType == vpiNegedgeOp and a single RefObj
//     operand naming "clk".
//   - the controlled statement is a blocking Assignment (lhs RefObj "a",
//     rhs Operation vpiBitNegOp("a")).
//
// Not checked:
//   - Runtime toggling behavior of "a" on a negedge -- HLC is an
...
namespace hlc {
class EventControlNegedgeTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "9.4.2--event_control_negedge.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

TEST_F(EventControlNegedgeTest, ModuleExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
}

TEST_F(EventControlNegedgeTest, ClkIsNetNotVariable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(EventControlNegedgeTest, NegedgeEventControlConditionReferencesClk) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  ...
  ASSERT_NE(eventControl, nullptr) << "always @(negedge clk) must produce an EventControl (9.4.2 event_control)";

  const hldb::Operation *const condition = any_cast<hldb::Operation>(eventControl->getCondition());
  ASSERT_NE(condition, nullptr);
  EXPECT_EQ(condition->getOpType(), vpiNegedgeOp);
...
TEST_F(EventControlNegedgeTest, ControlledStatementIsBlockingBitNegAssignment) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(EventControlNegedgeTest, RuntimeToggleShouldFlipAButHlcCannotSimulate) {
  GTEST_SKIP() << "HLC has no simulator (see .claude/hlc_overview.md); the effect of a negedge "
  ...
  EXPECT_NE(value->getDecompile(), "0") << "after one negedge 'a' should have toggled away from 0";
}
}  // namespace hlc
