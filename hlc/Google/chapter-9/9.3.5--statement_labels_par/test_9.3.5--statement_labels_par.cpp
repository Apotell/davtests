...
// tests/Google/chapter-9/9.3.5--statement_labels_par.sv
// ----------------------------------------------------------------------------
...
// ============================================================================
//
// IEEE 1800-2023 construct under test (Sec 9.3.5, "Statement labels"): "Any
// statement can be given a name by placing a label, followed by a colon,
// prior to the statement... this is functionally identical to naming the
// block". Here the label "name:" is placed BEFORE the "fork" keyword
// (generic statement-label form), rather than immediately after it as
// "fork : name" (the block-naming form of Sec 9.3.4, already checked in
// test_9.3.4--block_names_par.cpp). Per the standard's "functionally
// identical" wording, both forms should produce the same ForkStmt shape.
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
//   - module "block_tb" exists.
//   - the initial statement binds a ForkStmt directly, with
//     getJoinType() == vpiJoin.
//   - despite "name:" preceding "fork" rather than following it, the
//     ForkStmt's own name (Scope::getName()) is still "name" -- confirming
//     IEEE 1800-2023's claim that the generic statement-label form is
//     functionally identical to the Sec 9.3.4 block-naming form.
//   - the ForkStmt's getEndLabel() is "name" -- the repeated identifier
//     from "join : name".
...
namespace hlc {
class StatementLabelsParTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "9.3.5--statement_labels_par.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to StatementLabelsParTest go here!

TEST_F(StatementLabelsParTest, ModuleBlockTbExists) {
...
TEST_F(StatementLabelsParTest, LeadingStatementLabelProducesSameForkStmtNameAsBlockNamingForm) {
  ...
  ASSERT_NE(init->getStmt(), nullptr) << "'initial name: fork ... join: name' should bind the ForkStmt directly "
                                         "as the Initial's statement";
