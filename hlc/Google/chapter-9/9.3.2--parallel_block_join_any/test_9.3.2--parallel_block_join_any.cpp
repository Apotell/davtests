...
// Tests for 9.3.2--parallel_block_join_any.sv (tags: 9.3.2)
//   module parallel_tb ();
//     reg a = 0;
//     reg b = 0;
//     reg c = 0;
...
//   endmodule
//
// IEEE 1800-2017 Sec 9.3.2 "Parallel blocks (fork-join)": "join_any"
// (unlike the sibling 9.3.2--parallel_block_join.sv's plain "join") means
// the parent process resumes as soon as ANY ONE of the fork's statements
// completes, not all of them -- a distinct join keyword from "join", but
// the same ForkStmt (Scope) node structure, distinguished purely by
// getJoinType() (vpiJoinAny here, vs. vpiJoin). "fork...join_any" is
// itself the single statement following "initial" (no begin/end needed),
// so it sits directly on the Initial's stmt.
//
// "reg a/b/c = 0" -- same reasoning as 9.3.2--parallel_block_join.sv: each
// a Variable, single-bit unsigned LogicTypespec, no declared Ranges. All
...
//   - the ForkStmt's getJoinType() is vpiJoinAny; it has exactly 3
//     statements
...
class ParallelBlockJoinAnyTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "9.3.2--parallel_block_join_any.hlc"}); }
...
TEST_F(ParallelBlockJoinAnyTest, ModuleExists) { EXPECT_NE(getModule(), nullptr); }

TEST_F(ParallelBlockJoinAnyTest, ModuleHasThreeVariables) {
  const hldb::Module *const mod = getModule();
...
TEST_F(ParallelBlockJoinAnyTest, VariableAExists) { EXPECT_NE(getVariable("a"), nullptr); }
TEST_F(ParallelBlockJoinAnyTest, VariableBExists) { EXPECT_NE(getVariable("b"), nullptr); }
TEST_F(ParallelBlockJoinAnyTest, VariableCExists) { EXPECT_NE(getVariable("c"), nullptr); }

// "reg a/b/c" have no net-type keyword, so per IEEE 1800-2023 Sec 6.7/6.8
// none must also appear in the module's net collection.
TEST_F(ParallelBlockJoinAnyTest, VariablesAreNotDuplicatedAsNets) {
  const hldb::Module *const mod = getModule();
  ASSERT_NE(mod, nullptr);
...
TEST_F(ParallelBlockJoinAnyTest, VariableTypespecsAreLogicUnsignedWithNoRanges) {
...
TEST_F(ParallelBlockJoinAnyTest, AllVariablesInitialValueIsConstantZero) {
  for (std::string_view name : {"a", "b", "c"}) {
    const hldb::Variable *const v = getVariable(name);
    ASSERT_NE(v, nullptr);
...
TEST_F(ParallelBlockJoinAnyTest, ModuleHasOneProcess) {
  const hldb::Module *const mod = getModule();
  ASSERT_NE(mod, nullptr);
...
TEST_F(ParallelBlockJoinAnyTest, TheOneProcessIsInitial) { EXPECT_NE(getInitialProcess(), nullptr); }

// 9.3.2: "fork...join_any" is itself the single statement following
// "initial" -- no enclosing Begin is needed or expected.
TEST_F(ParallelBlockJoinAnyTest, InitialStmtIsDirectlyAForkStmt) {
  EXPECT_NE(getForkStmt(), nullptr) << "9.3.2: 'fork...join_any' must be modeled as a ForkStmt";
}

TEST_F(ParallelBlockJoinAnyTest, ForkJoinTypeIsJoinAny) {
  const hldb::ForkStmt *const fork = getForkStmt();
  ASSERT_NE(fork, nullptr);
  EXPECT_EQ(fork->getJoinType(), vpiJoinAny) << "9.3.2: 'join_any' must have JoinType vpiJoinAny";
}

TEST_F(ParallelBlockJoinAnyTest, ForkHasThreeStmts) {
  const hldb::ForkStmt *const fork = getForkStmt();
  ASSERT_NE(fork, nullptr);
...
TEST_F(ParallelBlockJoinAnyTest, FirstStmtIsBlockingAssignToA) {
...
TEST_F(ParallelBlockJoinAnyTest, SecondStmtIsBlockingAssignToB) {
...
TEST_F(ParallelBlockJoinAnyTest, ThirdStmtIsBlockingAssignToC) {
...
TEST_F(ParallelBlockJoinAnyTest, ReferencesAreNotFailedBinds) {
