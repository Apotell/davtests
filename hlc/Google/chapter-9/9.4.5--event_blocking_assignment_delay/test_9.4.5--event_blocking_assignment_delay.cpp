...
// Source under test: tests/Google/chapter-9/9.4.5--event_blocking_assignment_delay.sv
//
//   module block_tb ();
//   	reg a = 0;
//   	reg b = 1;
...
// (delay_or_event_control attached directly to a blocking_assignment,
// grammar: `variable_lvalue assignment_operator delay_or_event_control
// expression`). This is semantically different from a plain
// procedural-timing-control statement like "#10 a = b;" (9.4.1, tested in
// 9.4.1--delay_control.sv): "a = #10 b;" evaluates "b" NOW, then waits
// #10, THEN assigns the already-sampled value to "a" -- so the delay
// belongs to the Assignment node itself, not to a separate wrapping
// statement.
//
// Checked:
...
//     Constant "10"), NOT as a separate wrapping statement-level
//     DelayControl -- and Assignment::getEventControl() /
//     getRepeatControl() are both null, confirming exactly one of the
//     three delay_or_event_control alternatives is populated.
//
// Not checked:
//   - Runtime evaluation-then-delay ordering (is "b" really sampled at
//     time 0 and assigned to "a" only at time 10) -- HLC is an elaborator
...
class EventBlockingAssignmentDelayTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "9.4.5--event_blocking_assignment_delay.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

TEST_F(EventBlockingAssignmentDelayTest, ModuleExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
}

TEST_F(EventBlockingAssignmentDelayTest, AAndBAreVariables) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("block_tb", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(EventBlockingAssignmentDelayTest, AssignmentCarriesInlineDelayControl) {
  ...
  ASSERT_NE(assign, nullptr) << "'a = #10 b;' must be a single Assignment node, not a separate "
                                "wrapping DelayControl statement";
