...
//   endmodule
//
// This file is identical in content to chapter-7/unions/basic.sv, just
// declared under an explicit "unpacked" directory (a plain "union" with
// neither "packed" nor "tagged" is unpacked by default). The compiled HLDB
// structure below is the same in both.
//
// Checked:
...
//   - member "v2": typespec -> BitTypespec [3:0] vector
...
class UnionsUnpackedBasicTest : public Test {
 public:
...
TEST_F(UnionsUnpackedBasicTest, ModuleExists) { EXPECT_NE(getTop(), nullptr); }

TEST_F(UnionsUnpackedBasicTest, ModuleHasOneNet) {
  const hldb::Module *const top = getTop();
...
TEST_F(UnionsUnpackedBasicTest, UnIsUnpackedUntaggedUnionWithTwoMembers) {
...
TEST_F(UnionsUnpackedBasicTest, MemberV1IsEightBitBitTypespec) {
  const hldb::UnionTypespec *const ut = getUnUnionTypespec();
  ASSERT_NE(ut, nullptr);
  const hldb::Union *const u = ut->getUnion();
  ASSERT_NE(u, nullptr);
  ASSERT_NE(u->getMembers(), nullptr);
  const hldb::TypespecMember *const v1 = u->getMembers()->at(0);
  ASSERT_NE(v1, nullptr);
  EXPECT_EQ(v1->getName(), std::string_view("v1"));
...
TEST_F(UnionsUnpackedBasicTest, MemberV2IsFourBitBitTypespec) {
  const hldb::UnionTypespec *const ut = getUnUnionTypespec();
  ASSERT_NE(ut, nullptr);
  const hldb::Union *const u = ut->getUnion();
  ASSERT_NE(u, nullptr);
  ASSERT_NE(u->getMembers(), nullptr);
  const hldb::TypespecMember *const v2 = u->getMembers()->at(1);
  ASSERT_NE(v2, nullptr);
  EXPECT_EQ(v2->getName(), std::string_view("v2"));
...
TEST_F(UnionsUnpackedBasicTest, InitialBeginHasThreeStmts) {
...
TEST_F(UnionsUnpackedBasicTest, FirstStmtAssignsDecimalOneFourZeroToUnV1) {
  const hldb::Begin *const begin = getInitialBegin();
  ASSERT_NE(begin, nullptr);
...
TEST_F(UnionsUnpackedBasicTest, DesignHasStringTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_NE(any_cast<hldb::StringTypespec>(m_design->getTypespecs()->at(2)), nullptr);
}

TEST_F(UnionsUnpackedBasicTest, CompilerReportsZeroErrors) {
...
TEST_F(UnionsUnpackedBasicTest, NoContAssigns) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(UnionsUnpackedBasicTest, RuntimeUnionOverlapValuesRequireSimulation) {
  ...
  EXPECT_EQ(any_cast<hldb::Constant>(v2Display->getArguments()->at(0))->getValue(), std::string_view(":assert: (%d == 12)"))
