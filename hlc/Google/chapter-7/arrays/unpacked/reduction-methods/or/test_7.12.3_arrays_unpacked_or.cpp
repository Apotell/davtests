...
// Tests for or.sv (tags: 7.12.3 7.4.2)
//   module top ();
//     byte b[] = { 1, 2, 3, 4 };
//     int y;
//     initial begin
...
//
// Not checked:
//   - actual runtime value of y after y = b.or -- simulation-only (see
//     the skipped canary RuntimeOrResultRequiresSimulation below)
...
class UnpackedOrTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "or.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

// --- module / variables ----

TEST_F(UnpackedOrTest, ModuleExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  EXPECT_NE(top, nullptr);
}

TEST_F(UnpackedOrTest, ModuleHasTwoVariables) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
...
TEST_F(UnpackedOrTest, ModuleHasNoNets) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  EXPECT_EQ(top->getNets(), nullptr) << "no net-type keyword is present in or.sv";
}

TEST_F(UnpackedOrTest, VarBIsDynamicArrayOfSignedByte) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(UnpackedOrTest, VarBInitialValueIsConcatenationOfOneTwoThreeFour) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const b = hldb::findByName<hldb::Variable>("b", top->getVariables());
  ASSERT_NE(b, nullptr);
...
TEST_F(UnpackedOrTest, VarYIsPlainSignedIntWithNoInitialValue) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(UnpackedOrTest, InitialBeginHasThreeStmts) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(UnpackedOrTest, FirstStmtDisplaysOneTwoThreeFour) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(UnpackedOrTest, SecondStmtAssignsYFromBOrReduction) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Begin *const begin = any_cast<hldb::Initial>(top->getProcesses()->at(0))->getStmt<hldb::Begin>();
  ASSERT_NE(begin, nullptr);
  ...
  EXPECT_EQ(call->getName(), "or");
  EXPECT_EQ(call->getArguments(), nullptr);
}

TEST_F(UnpackedOrTest, ThirdStmtDisplaysYEqualsSeven) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Begin *const begin = any_cast<hldb::Initial>(top->getProcesses()->at(0))->getStmt<hldb::Begin>();
  ASSERT_NE(begin, nullptr);
...
TEST_F(UnpackedOrTest, DesignHasSignedIntTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
...
TEST_F(UnpackedOrTest, DesignHasStringTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_NE(any_cast<hldb::StringTypespec>(m_design->getTypespecs()->at(2)), nullptr);
}

TEST_F(UnpackedOrTest, CompilerReportsZeroErrors) {
  ASSERT_NE(m_session->getErrorContainer(), nullptr);
  const ErrorContainer::Stats stats = m_session->getErrorContainer()->getErrorStats();
  EXPECT_EQ(stats.nbFatal, 0);
  EXPECT_EQ(stats.nbSyntax, 0);
  EXPECT_EQ(findError(ErrorDefinition::COMP_FAILED_TO_BIND, "or"), nullptr)
      << "arr.or() must bind (IEEE 1800-2023 7.12.3)";
  EXPECT_EQ(stats.nbWarning, 0);
}

TEST_F(UnpackedOrTest, NoContAssigns) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  EXPECT_EQ(top->getContAssigns(), nullptr);
}

// --- known gap: runtime or-reduction result requires simulation ----

TEST_F(UnpackedOrTest, RuntimeOrResultRequiresSimulation) {
  GTEST_SKIP() << "This harness only compiles/elaborates or.sv; it does not run a simulator, so the "
                  "actual runtime value of y after y = b.or cannot be observed here. or.sv's own "
                  "$display format string documents the expected value.";
