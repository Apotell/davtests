...
// Tests for sum.sv (tags: 7.12.3 7.4.2)
//   module top ();
//     byte b[] = { 1, 2, 3, 4 };
//     int y;
//     initial begin
...
//     "y" resolving Variable "y", rhs RefObj "b.sum()" with 2 path
...
//
// Not checked:
//   - actual runtime value of y after y = b.sum -- simulation-only (see
//     the skipped canary RuntimeSumResultRequiresSimulation below)
...
class UnpackedSumTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "sum.hlc"}); }
...
TEST_F(UnpackedSumTest, DesignHasModuleTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
...
TEST_F(UnpackedSumTest, DesignHasStringTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
...
TEST_F(UnpackedSumTest, RuntimeSumResultRequiresSimulation) {
  ...
  ASSERT_NE(disp, nullptr);
  EXPECT_EQ(any_cast<hldb::Constant>(disp->getArguments()->at(0))->getValue(), ":assert: (%d == 10)")
      << "expected y == (1 + 2 + 3 + 4) == 10";
}
