...
// Tests for increment.sv (tags: 7.11 7.4.1)
//   module top ();
//     bit [7:0] arr;
//     initial begin
...
//     "$increment" with 1 argument -- RefObj "arr" resolving to Variable "arr"
...
//
//  checked:
//   - actual runtime value returned by $increment(arr) -- simulation-only
...
class PackedQueryIncrementTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "increment.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

// --- module / variables ----

TEST_F(PackedQueryIncrementTest, ModuleExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  EXPECT_NE(top, nullptr);
}

TEST_F(PackedQueryIncrementTest, ModuleHasOneVariable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
...
TEST_F(PackedQueryIncrementTest, VariableArrNameIsArr) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryIncrementTest, VariableArrTypespecIsBitWithRangeSevenToZero) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const arr = hldb::findByName<hldb::Variable>("arr", top->getVariables());
  ASSERT_NE(arr, nullptr);
...
TEST_F(PackedQueryIncrementTest, InitialBeginHasOneStmt) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryIncrementTest, DisplayCallHasTwoArgumentsFirstIsAssertString) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryIncrementTest, NestedIncrementCallHasArrArgument) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Begin *const begin = any_cast<hldb::Initial>(top->getProcesses()->at(0))->getStmt<hldb::Begin>();
  ASSERT_NE(begin, nullptr);
...
TEST_F(PackedQueryIncrementTest, DesignHasSignedIntTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
...
TEST_F(PackedQueryIncrementTest, DesignHasStringTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_NE(any_cast<hldb::StringTypespec>(m_design->getTypespecs()->at(2)), nullptr);
}

TEST_F(PackedQueryIncrementTest, CompilerReportsZeroErrors) {
...
TEST_F(PackedQueryIncrementTest, NoContAssigns) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  EXPECT_EQ(top->getContAssigns(), nullptr);
}

// --- known gap: runtime $increment value requires simulation ----

TEST_F(PackedQueryIncrementTest, RuntimeIncrementValueRequiresSimulation) {
  // GTEST_SKIP() << "This harness only compiles/elaborates increment.sv; it does not run a simulator, "
  //                 "so the actual runtime value returned by $increment(arr) cannot be observed here. "
  //                 "increment.sv's own $display format string documents the expected value.";
