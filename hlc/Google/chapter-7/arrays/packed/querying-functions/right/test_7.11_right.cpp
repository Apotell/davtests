...
// Tests for right.sv (tags: 7.11 7.4.1)
//   module top ();
//     bit [7:0] arr;
//     initial begin
...
//     (vpiConstType=string(6), size=144) and a nested SysFuncCall "$right"
...
//
//  checked:
//   - actual runtime value returned by $right(arr) -- simulation-only 
...
class PackedQueryRightTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "right.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

// --- module / variables ----

TEST_F(PackedQueryRightTest, ModuleExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  EXPECT_NE(top, nullptr);
}

TEST_F(PackedQueryRightTest, ModuleHasOneVariable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
...
TEST_F(PackedQueryRightTest, VariableArrNameIsArr) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryRightTest, VariableArrTypespecIsBitWithRangeSevenToZero) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const arr = hldb::findByName<hldb::Variable>("arr", top->getVariables());
  ASSERT_NE(arr, nullptr);
...
TEST_F(PackedQueryRightTest, InitialBeginHasOneStmt) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryRightTest, DisplayCallHasTwoArgumentsFirstIsAssertString) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryRightTest, NestedRightCallHasArrArgument) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Begin *const begin = any_cast<hldb::Initial>(top->getProcesses()->at(0))->getStmt<hldb::Begin>();
  ASSERT_NE(begin, nullptr);
...
TEST_F(PackedQueryRightTest, DesignHasThreeTypespecs) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_EQ(m_design->getTypespecs()->size(), 3u);
}

TEST_F(PackedQueryRightTest, DesignHasModuleTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
...
TEST_F(PackedQueryRightTest, DesignHasSignedIntTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
...
TEST_F(PackedQueryRightTest, DesignHasStringTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_NE(any_cast<hldb::StringTypespec>(m_design->getTypespecs()->at(2)), nullptr);
}

TEST_F(PackedQueryRightTest, CompilerReportsZeroErrors) {
...
TEST_F(PackedQueryRightTest, NoContAssigns) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  EXPECT_EQ(top->getContAssigns(), nullptr);
}

// --- known gap: runtime $right value requires simulation ----

TEST_F(PackedQueryRightTest, RuntimeRightValueRequiresSimulation) {
  // GTEST_SKIP() << "This harness only compiles/elaborates right.sv; it does not run a simulator, so "
  //                 "the actual runtime value returned by $right(arr) cannot be observed here. right.sv's "
