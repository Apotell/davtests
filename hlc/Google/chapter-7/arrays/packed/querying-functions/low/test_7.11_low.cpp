...
// Tests for low.sv (tags: 7.11 7.4.1)
//   module top ();
//     bit [7:0] arr;
//     initial begin
...
//     (vpiConstType=string(6), size=144) and a nested SysFuncCall "$low"
...
//
//  checked:
//   - actual runtime value returned by $low(arr) -- simulation-only 
...
class PackedQueryLowTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "low.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

// --- module / variables ----

TEST_F(PackedQueryLowTest, ModuleExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  EXPECT_NE(top, nullptr);
}

TEST_F(PackedQueryLowTest, ModuleHasOneVariable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
...
TEST_F(PackedQueryLowTest, VariableArrNameIsArr) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryLowTest, VariableArrTypespecIsBitWithRangeSevenToZero) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const arr = hldb::findByName<hldb::Variable>("arr", top->getVariables());
  ASSERT_NE(arr, nullptr);
...
TEST_F(PackedQueryLowTest, InitialBeginHasOneStmt) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryLowTest, DisplayCallHasTwoArgumentsFirstIsAssertString) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PackedQueryLowTest, NestedLowCallHasArrArgument) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Begin *const begin = any_cast<hldb::Initial>(top->getProcesses()->at(0))->getStmt<hldb::Begin>();
  ASSERT_NE(begin, nullptr);
  const hldb::SysTaskCall *const disp = any_cast<hldb::SysTaskCall>(begin->getStmts()->at(0));
  ASSERT_NE(disp, nullptr);
  const hldb::SysFuncCall *const low = any_cast<hldb::SysFuncCall>(disp->getArguments()->at(1));
  ASSERT_NE(low, nullptr);
  EXPECT_EQ(low->getName(), "$low");
  ASSERT_NE(low->getArguments(), nullptr);
  ASSERT_EQ(low->getArguments()->size(), 1u);
  const hldb::RefObj *const arg = any_cast<hldb::RefObj>(low->getArguments()->at(0));
...
TEST_F(PackedQueryLowTest, DesignHasSignedIntTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
...
TEST_F(PackedQueryLowTest, DesignHasStringTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_NE(any_cast<hldb::StringTypespec>(m_design->getTypespecs()->at(2)), nullptr);
}

TEST_F(PackedQueryLowTest, CompilerReportsZeroErrors) {
...
TEST_F(PackedQueryLowTest, NoContAssigns) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  EXPECT_EQ(top->getContAssigns(), nullptr);
}

// --- known gap: runtime $low value requires simulation ----

TEST_F(PackedQueryLowTest, RuntimeLowValueRequiresSimulation) {
  // GTEST_SKIP() << "This harness only compiles/elaborates low.sv; it does not run a simulator, so "
  //                 "the actual runtime value returned by $low(arr) cannot be observed here. low.sv's "
  ...
  EXPECT_EQ(any_cast<hldb::Constant>(disp->getArguments()->at(0))->getValue(), ":assert: (%d == 0)")
      << "expected $low(arr) == 0 since arr's packed range [7:0] has 0 as the lower-numbered index";
}
