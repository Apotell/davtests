...
//       not as a method call; Surelog emits EL0535 "Illegal implicit net"
...
//
// Not checked:
//   - SysFuncCall::getFuncType() (not set by Surelog for $display)
...
class OpDeleteTest : public Test {
 public:
  static void SetUpTestSuite() {
    Compile(__FILE__, {"-f", "op-delete.hlc"});
...
TEST_F(OpDeleteTest, BitTypespecIsVector) {
  const hldb::Module *const top =
      hldb::findByName<hldb::Module>("work@top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::BitTypespec *const bt =
      top->getNets()->at(0)->getTypespec<hldb::RefTypespec>()
          ->getActual<hldb::ArrayTypespec>()
          ->getElemTypespec()->getActual<hldb::BitTypespec>();
...
TEST_F(OpDeleteTest, RangeLeftIs7) {
  ...
  const hldb::Constant *const left =
      bt->getRanges()->at(0)->getLeftExpr<hldb::Constant>();
...
TEST_F(OpDeleteTest, RangeRightIs0) {
  ...
  const hldb::Constant *const right =
      bt->getRanges()->at(0)->getRightExpr<hldb::Constant>();
...
TEST_F(OpDeleteTest, AssignmentIsBlocking) {
  ...
  const hldb::Assignment *const assign =
      any_cast<hldb::Assignment>(
          any_cast<hldb::Initial>(top->getProcesses()->at(0))
              ->getStmt<hldb::Begin>()->getStmts()->at(0));
...
TEST_F(OpDeleteTest, ArrayExprSizeIs16) {
  ...
  const hldb::Constant *const c =
      any_cast<hldb::Constant>(ae->getExprs()->at(0));
  ASSERT_NE(c, nullptr);
...
TEST_F(OpDeleteTest, FirstDisplayNameIsDisplay) {
  const hldb::Module *const top =
      hldb::findByName<hldb::Module>("work@top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::SysFuncCall *const call =
      any_cast<hldb::SysFuncCall>(
          any_cast<hldb::Initial>(top->getProcesses()->at(0))
              ->getStmt<hldb::Begin>()->getStmts()->at(1));
...
TEST_F(OpDeleteTest, FirstDisplaySecondArgIsArrSizeHierPath) {
  ...
  const hldb::HierPath *const hp =
      any_cast<hldb::HierPath>(call->getArguments()->at(1));
...
TEST_F(OpDeleteTest, ThirdStmtIsHierPath) {
  ...
  // Surelog emits EL0535 and stores it as a HierPath statement
  const hldb::Module *const top =
      hldb::findByName<hldb::Module>("work@top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Begin *const begin =
...
TEST_F(OpDeleteTest, DeleteHierPathNameIsArrDelete) {
  ...
  ASSERT_NE(top, nullptr);
  const hldb::HierPath *const hp =
      any_cast<hldb::HierPath>(
          any_cast<hldb::Initial>(top->getProcesses()->at(0))
              ->getStmt<hldb::Begin>()->getStmts()->at(2));
  ASSERT_NE(hp, nullptr);
...
TEST_F(OpDeleteTest, DeleteHierPathSecondElemIsDelete) {
  ...
  ASSERT_NE(hp->getPathElems(), nullptr);
  const hldb::RefObj *const elem =
      any_cast<hldb::RefObj>(hp->getPathElems()->at(1));
  ASSERT_NE(elem, nullptr);
...
TEST_F(OpDeleteTest, SecondDisplayNameIsDisplay) {
  ...
  const hldb::SysFuncCall *const call =
      any_cast<hldb::SysFuncCall>(
          any_cast<hldb::Initial>(top->getProcesses()->at(0))
              ->getStmt<hldb::Begin>()->getStmts()->at(3));
  ASSERT_NE(call, nullptr);
  EXPECT_EQ(call->getName(), "$display");
}
...
TEST_F(OpDeleteTest, DesignHasIntTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_NE(
      any_cast<hldb::IntTypespec>(m_design->getTypespecs()->at(1)), nullptr);
}
...
TEST_F(OpDeleteTest, DesignHasStringTypespec) {
  ...
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_NE(
      any_cast<hldb::StringTypespec>(m_design->getTypespecs()->at(2)), nullptr);
}
