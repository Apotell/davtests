...
// Tests for push_back.sv (tags: 7.10.2.7 7.10.2)
//   module top ();
//     int q[$];
//     initial begin
//       q.push_back(4);
...
//   endmodule
//
// IEEE 1800-2017 7.10.2.7 "push_back(item)": inserts "item" at the back
// (highest index) of the queue, growing it by one element without
// disturbing the elements already present. Pushing 4, then 3, then 2
// leaves the queue as {4, 3, 2} -- the first-pushed value "4" stays at
// the front (index 0).
//
// Checked:
...
//     Constant "0" -- confirms the front element is unaffected by
//     subsequent push_back calls
...
//
// KNOWN COMPILER BUG (not a defect in push_back.sv):
...
//   chapter-7/queues/pop_front/pop_front.sv and chapter-7/queues/
//   pop_front_assign/pop_front_assign.sv). That the parenthesized form
//   works is independently verified by
...
class QueuesPushBackTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "push_back.hlc"}); }
...
TEST_F(QueuesPushBackTest, NetQArrayIsNotPacked) {
  const hldb::ArrayTypespec *const at = getQArrayTypespec();
  ASSERT_NE(at, nullptr);
  EXPECT_FALSE(at->getPacked()) << "a queue dimension is an unpacked dimension";
}

TEST_F(QueuesPushBackTest, NetQRangeLeftIsUnboundedDollar) {
  const hldb::ArrayTypespec *const at = getQArrayTypespec();
  ASSERT_NE(at, nullptr);
...
TEST_F(QueuesPushBackTest, NetQElemTypespecIsSignedIntTypespec) {
  const hldb::ArrayTypespec *const at = getQArrayTypespec();
  ASSERT_NE(at, nullptr);
...
TEST_F(QueuesPushBackTest, InitialBeginHasFiveStmts) {
  const hldb::Begin *const begin = getInitialBegin();
  ASSERT_NE(begin, nullptr);
  ASSERT_NE(begin->getStmts(), nullptr);
...
TEST_F(QueuesPushBackTest, FifthStmtDisplayAssertsQAtZeroEqualsFour) {
  ...
  ASSERT_NE(sel, nullptr) << "'q[0]' should be a BitSelect -- confirms push_back does not disturb the front element";
...
TEST_F(QueuesPushBackTest, DesignHasStringTypespec) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  ASSERT_GT(m_design->getTypespecs()->size(), 2u);
  EXPECT_NE(any_cast<hldb::StringTypespec>(m_design->getTypespecs()->at(2)), nullptr);
}

TEST_F(QueuesPushBackTest, NoBindErrorForSize) {
