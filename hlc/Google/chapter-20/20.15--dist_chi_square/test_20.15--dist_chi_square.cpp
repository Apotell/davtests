...
// Tests for 20.15--dist_chi_square.sv (tags: 20.15)
//   module top();
//   initial begin
//   	integer seed = 1234;
...
// "function integer $dist_chi_square (inout integer seed, input integer
// df);" -- returns a pseudo-random number with a chi-square distribution,
// updating seed by reference and taking the degree of freedom (df) as its
// second argument.
//
// Checked:
...
//     "$dist_chi_square"
//   - the "$dist_chi_square" SysFuncCall has exactly 2 arguments: a RefObj
...
// $dist_chi_square) cannot be observed -- HLC is a compiler/elaborator with
...
class DistChiSquareFunctionTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "20.15--dist_chi_square.hlc"}); }
 ...
 protected:
  ...
  static const hldb::SysFuncCall *getDistChiSquareCall() {
...
TEST_F(DistChiSquareFunctionTest, ModuleExists) { EXPECT_NE(getModule(), nullptr); }

TEST_F(DistChiSquareFunctionTest, ModuleHasOneInitialProcess) {
  const hldb::Module *const mod = getModule();
...
TEST_F(DistChiSquareFunctionTest, InitialBodyIsBeginWithOneVariableAndOneStmt) {
  const hldb::Begin *const body = getInitialBody();
...
TEST_F(DistChiSquareFunctionTest, SeedIsSignedIntegerVariable) {
...
TEST_F(DistChiSquareFunctionTest, SeedHasInitialValue1234) {
  const hldb::Variable *const seed = getSeedVariable();
...
TEST_F(DistChiSquareFunctionTest, StmtIsDisplaySysTaskCall) {
...
TEST_F(DistChiSquareFunctionTest, DisplayCallHasFormatAndDistChiSquareArgument) {
  const hldb::SysTaskCall *const call = getDisplayCall();
  ...
  EXPECT_NE(getDistChiSquareCall(), nullptr) << "'$dist_chi_square(...)' should be a SysFuncCall";
}

TEST_F(DistChiSquareFunctionTest, DistChiSquareCallIsNamedCorrectly) {
  const hldb::SysFuncCall *const call = getDistChiSquareCall();
  ASSERT_NE(call, nullptr);
  EXPECT_EQ(call->getName(), "$dist_chi_square");
}

TEST_F(DistChiSquareFunctionTest, DistChiSquareCallHasSeedAndDfArguments) {
  const hldb::SysFuncCall *const call = getDistChiSquareCall();
  ASSERT_NE(call, nullptr);
  ASSERT_NE(call->getArguments(), nullptr);
  ASSERT_EQ(call->getArguments()->size(), 2u)
...
TEST_F(DistChiSquareFunctionTest, CompilesWithNoErrors) {
