...
// Tests for 20.15--dist_erlang.sv (tags: 20.15)
//   module top();
//   initial begin
//   	integer seed = 1234;
...
// "function integer $dist_erlang (inout integer seed, input integer k,
// input integer mean);" -- returns a pseudo-random number with an Erlang
// distribution, updating seed by reference and taking the shape parameter
// (k) and mean as its second and third arguments.
//
// Checked:
...
//     "$dist_erlang"
//   - the "$dist_erlang" SysFuncCall has exactly 3 arguments: a RefObj
//     "seed" resolving to the Variable "seed", a Constant unsigned int "3"
//     (k), and a Constant unsigned int "100" (mean)
//   - compiler reports zero errors
//
// NOT CHECKED: runtime effects (the actual pseudo-random value returned by
// $dist_erlang) cannot be observed -- HLC is a compiler/elaborator with no
...
class DistErlangFunctionTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "20.15--dist_erlang.hlc"}); }
 ...
 protected:
  ...
  static const hldb::SysFuncCall *getDistErlangCall() {
...
TEST_F(DistErlangFunctionTest, ModuleExists) { EXPECT_NE(getModule(), nullptr); }

TEST_F(DistErlangFunctionTest, ModuleHasOneInitialProcess) {
  const hldb::Module *const mod = getModule();
...
TEST_F(DistErlangFunctionTest, InitialBodyIsBeginWithOneVariableAndOneStmt) {
  const hldb::Begin *const body = getInitialBody();
...
TEST_F(DistErlangFunctionTest, SeedIsSignedIntegerVariable) {
...
TEST_F(DistErlangFunctionTest, SeedHasInitialValue1234) {
  const hldb::Variable *const seed = getSeedVariable();
...
TEST_F(DistErlangFunctionTest, StmtIsDisplaySysTaskCall) {
...
TEST_F(DistErlangFunctionTest, DisplayCallHasFormatAndDistErlangArgument) {
  const hldb::SysTaskCall *const call = getDisplayCall();
  ...
  EXPECT_NE(getDistErlangCall(), nullptr) << "'$dist_erlang(...)' should be a SysFuncCall";
}

TEST_F(DistErlangFunctionTest, DistErlangCallIsNamedCorrectly) {
  const hldb::SysFuncCall *const call = getDistErlangCall();
  ASSERT_NE(call, nullptr);
  EXPECT_EQ(call->getName(), "$dist_erlang");
}

TEST_F(DistErlangFunctionTest, DistErlangCallHasSeedKAndMeanArguments) {
  const hldb::SysFuncCall *const call = getDistErlangCall();
  ASSERT_NE(call, nullptr);
  ASSERT_NE(call->getArguments(), nullptr);
  ASSERT_EQ(call->getArguments()->size(), 3u)
      << "20.15: '$dist_erlang' takes an inout seed and input k, mean arguments";
  ...
  const hldb::Constant *const k = any_cast<hldb::Constant>(call->getArguments()->at(1));
  ASSERT_NE(k, nullptr) << "'3' should be a Constant";
  EXPECT_EQ(k->getConstType(), vpiUIntConst);
  EXPECT_EQ(k->getDecompile(), "3");

  const hldb::Constant *const mean = any_cast<hldb::Constant>(call->getArguments()->at(2));
...
TEST_F(DistErlangFunctionTest, CompilesWithNoErrors) {
