...
// tests/Google/chapter-16/16.12--property-prec.sv
// ----------------------------------------------------------------------------
...
// non-overlapped implication"): `assert property (@(posedge clk) a |-> b);`
// -- a property_spec written inline, using the overlapped implication
// operator "|->" directly on plain signals (no local variables, unlike
// test_16.10--property-local-var.cpp's "|->").
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
//   - module "top" exists.
...
//     vpiOverlapImplyOp ("|->"), with two operands referencing "a" and "b".
//
...
class PropertyPrecTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.12--property-prec.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to PropertyPrecTest go here!

TEST_F(PropertyPrecTest, ModuleTopExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr) << "module 'top' not found";
}

TEST_F(PropertyPrecTest, AssertPropertyIsReachable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
...
TEST_F(PropertyPrecTest, PropertySpecHasPosedgeClockingEventAndNoDisableCondition) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PropertyPrecTest, PropertyExprIsOverlappedImplicationOfAAndB) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Assert *const assertProp = any_cast<hldb::Assert>(top->getConcurrentAssertions()->front());
  ASSERT_NE(assertProp, nullptr);
  ...
  EXPECT_TRUE(OperandsContainNamedRef(implyOp, "a")) << "'a' should be an operand of '|->'";
