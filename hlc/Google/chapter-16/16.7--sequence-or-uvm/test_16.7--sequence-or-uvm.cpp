...
// tests/Google/chapter-16/16.7--sequence-or-uvm.sv
// ----------------------------------------------------------------------------
...
// is the "or" counterpart of test_16.7--sequence-and-uvm.cpp -- same
// clocking event and same two cycle-delayed sub-sequences, but combined
// with the "or" sequence operator instead of "and", and with no outer
// "##0" wrapper this time (the "or" expression is itself the whole
// sequence body):
//   (dif.req ##5 dif.gnt0)  or  (dif.req ##3 dif.gnt1)
//
// Per the VPI object model, sequence "or" is modeled by vpiCompOrOp (the
// composite counterpart to vpiCompAndOp used for "and" in
...
//     vpiCompOrOp (no outer cycle-delay wrapper, unlike the "and" file's
//     "##0"), with exactly two operands.
//   - each of those two operands is itself an Operation with opType ==
//     vpiUnaryCycleDelayOp, each referencing "req", with delay magnitudes
//     "5" and "3" respectively present as Constants.
...
//   - The exact operand COUNT/ORDER within each cycle-delay/"or" Operation
//     (same open question as test_16.7--sequence-and-uvm.cpp) -- checks
//     search the operand list rather than index into a fixed position.
...
class SequenceOrUvmTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.7--sequence-or-uvm.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to SequenceOrUvmTest go here!

TEST_F(SequenceOrUvmTest, ModuleModAndTopAndClassEnvExist) {
...
TEST_F(SequenceOrUvmTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(SequenceOrUvmTest, SequenceSeqDeclaredInModuleTop) {
...
TEST_F(SequenceOrUvmTest, SequenceExprIsClockedSeqWithPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::SequenceDecl *const seq = hldb::findByName<hldb::SequenceDecl>("seq", top->getSequenceDecls());
...
TEST_F(SequenceOrUvmTest, SequenceExprIsOrOfTwoCycleDelaySubsequences) {
  // Unconfirmed (2026-09-08): vpiCompOrOp/Constant shapes resolve fine;
  // only the nested cycle-delay opType != vpiUnaryCycleDelayOp is
  ...
  ASSERT_NE(clocked->getSequenceExpr(), nullptr) << "'(dif.req ##5 dif.gnt0) or (dif.req ##3 dif.gnt1)' is "
                                                     "the sequence body";
  const hldb::Operation *const orOp = any_cast<hldb::Operation>(clocked->getSequenceExpr());
  ASSERT_NE(orOp, nullptr) << "the sequence body should directly be an Operation (no outer cycle-delay "
                              "wrapper, unlike the 'and' file's trailing '##0')";
  EXPECT_EQ(orOp->getOpType(), vpiCompOrOp) << "sequence 'or' should be vpiCompOrOp";

  ASSERT_NE(orOp->getOperands(), nullptr);
  ASSERT_EQ(orOp->getOperands()->size(), 2u) << "sequence 'or' combines exactly two sub-sequences";

  int32_t cycleDelaySubsequences = 0;
  for (const hldb::Any *const operand : *orOp->getOperands()) {
    if (const hldb::Operation *const sub = any_cast<hldb::Operation>(operand)) {
      if (sub->getOpType() == vpiUnaryCycleDelayOp) {
        ...
        EXPECT_TRUE(OperandsContainNamedRef(sub, "req")) << "each 'or' operand should reference 'dif.req'";
      }
    }
  }
  EXPECT_EQ(cycleDelaySubsequences, 2)
      << "both 'dif.req ##5 dif.gnt0' and 'dif.req ##3 dif.gnt1' should be vpiUnaryCycleDelayOp Operations";

  EXPECT_TRUE(OperandsContainConstant(any_cast<hldb::Operation>(orOp->getOperands()->at(0)), "5") ||
              OperandsContainConstant(any_cast<hldb::Operation>(orOp->getOperands()->at(1)), "5"))
      << "one 'or' operand should carry the '##5' delay magnitude";
  EXPECT_TRUE(OperandsContainConstant(any_cast<hldb::Operation>(orOp->getOperands()->at(0)), "3") ||
              OperandsContainConstant(any_cast<hldb::Operation>(orOp->getOperands()->at(1)), "3"))
      << "the other 'or' operand should carry the '##3' delay magnitude";
}

TEST_F(SequenceOrUvmTest, AssertPropertyIsReachableAndReferencesSeq) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
