...
// tests/Google/chapter-16/16.7--sequence-uvm-fail.sv
// ----------------------------------------------------------------------------
...
// test_16.7--sequence-uvm.cpp -- same ClockedSeq/posedge-clocking-event/
// single-cycle-delay shape -- except both sides of the "##1" reference the
// same signal, "read" ('dif.read ##1 dif.read' instead of
// 'dif.read ##1 dif.write'). Per the source's own :should_fail_because:
// comment, the DUT actually interleaves read/write, so "read" does not
// stay asserted two cycles in a row at runtime -- a functional/simulation
// defect, invisible to this compile-only object model (same reasoning as
// test_16.2--assume-uvm-fail.cpp for its DUT bug).
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
//     with a Constant "1" delay magnitude, and a "read" reference present
//     among its operands (both sides use the same signal name here, so
//     unlike test_16.7--sequence-uvm.cpp this file does not separately
//     check for two distinct signal names).
...
//   - The "mem_ctrl" DUT's interleaved read/write logic that actually
//     causes the runtime failure is not inspected -- confirming it requires
...
//     actually fails because of the interleaving -- cannot be observed: HLC
...
class SequenceUvmFailTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.7--sequence-uvm-fail.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to SequenceUvmFailTest go here!

TEST_F(SequenceUvmFailTest, ModuleMemCtrlAndTopAndClassEnvExist) {
...
TEST_F(SequenceUvmFailTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(SequenceUvmFailTest, SequenceSeqDeclaredInModuleTop) {
...
TEST_F(SequenceUvmFailTest, SequenceExprIsClockedSeqWithPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::SequenceDecl *const seq = hldb::findByName<hldb::SequenceDecl>("seq", top->getSequenceDecls());
...
TEST_F(SequenceUvmFailTest, SequenceExprIsSingleCycleDelayOnReadSignal) {
  ...
  ASSERT_NE(clocked->getSequenceExpr(), nullptr) << "'dif.read ##1 dif.read' is the sequence body";
  const hldb::Operation *const op = any_cast<hldb::Operation>(clocked->getSequenceExpr());
  ASSERT_NE(op, nullptr) << "'dif.read ##1 dif.read' should be an Operation";
...
TEST_F(SequenceUvmFailTest, AssertPropertyIsReachableAndReferencesSeq) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
