...
// tests/Google/chapter-16/16.17--expect-uvm-fail.sv
// ----------------------------------------------------------------------------
...
// ============================================================================
//
// IEEE 1800-2023 constructs under test: same "expect" statement and DUT
// shape as test_16.17--expect-uvm.cpp, but the delay changes from "##1" to
// "##2" ("dif.read ##2 dif.write" instead of "dif.read ##1 dif.write") --
// the property itself differs here, not just the DUT (same pattern as
// test_16.12--property-prec-uvm-fail.cpp's "read |=> read" vs.
// "read |=> write"). Per the source's own :should_fail_because: comment,
// "read" is actually asserted every 2 cycles by the DUT's phase toggling,
// which does not line up with a "##2"-delayed check against "write" --
// a functional/simulation-time outcome, invisible to this compile-only
// object model.
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
//     vpiUnaryCycleDelayOp ("##2"), with a Constant "2" delay magnitude
//     (not "1" -- matching this file's different delay), referencing
//     "read" and "write".
...
//     the runtime mismatch) is not inspected.
...
class ExpectUvmFailTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.17--expect-uvm-fail.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to ExpectUvmFailTest go here!

TEST_F(ExpectUvmFailTest, ModuleMemCtrlAndTopAndClassEnvExist) {
...
TEST_F(ExpectUvmFailTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(ExpectUvmFailTest, ExpectStmtIsPresentInModuleTop) {
...
TEST_F(ExpectUvmFailTest, ExpectStmtPropertySpecHasPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::ExpectStmt *const expectStmt = FindExpectStmtInModule(top);
  ASSERT_NE(expectStmt, nullptr);

  ASSERT_NE(expectStmt->getPropertySpec(), nullptr) << "'@(posedge dif.clk) dif.read ##2 dif.write' should "
...
TEST_F(ExpectUvmFailTest, PropertyExprIsCycleDelayOfTwoOfReadAndWrite) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::ExpectStmt *const expectStmt = FindExpectStmtInModule(top);
  ASSERT_NE(expectStmt, nullptr);
  ...
  ASSERT_NE(expectStmt->getPropertySpec()->getPropertyExpr(), nullptr)
      << "'dif.read ##2 dif.write' is the property expression";
  const hldb::Operation *const delayOp = any_cast<hldb::Operation>(expectStmt->getPropertySpec()->getPropertyExpr());
  ASSERT_NE(delayOp, nullptr) << "'dif.read ##2 dif.write' should be an Operation";
  EXPECT_EQ(delayOp->getOpType(), vpiUnaryCycleDelayOp) << "'##2' is a binary cycle delay (see file header "
                                                            "for the vpiUnaryCycleDelayOp-vs-vpiCycleDelayOp "
                                                            "open question)";
  EXPECT_TRUE(OperandsContainConstant(delayOp, "2")) << "the delay magnitude, '2', should be present (not "
                                                         "'1' -- this file's property uses a different delay "
                                                         "than test_16.17--expect-uvm.cpp)";
...
TEST_F(ExpectUvmFailTest, ExpectStmtHasNoPassStmtButHasElseStmt) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::ExpectStmt *const expectStmt = FindExpectStmtInModule(top);
  ASSERT_NE(expectStmt, nullptr);
