...
// tests/Google/chapter-16/16.14--assume-property-uvm-fail.sv
// ----------------------------------------------------------------------------
...
//         if(phase) begin
//             read <= 1;
//             write <= 1;                          // bug: both asserted together
//         end
...
// test_16.14--assume-property-uvm.cpp -- the same
// "assume property (@(posedge dif.clk) !(dif.read & dif.write)) else ...;".
// Per the source's own :should_fail_because: comment, this file's
// "mem_ctrl" DUT asserts "read" and "write" together in its "if(phase)"
// branch (a real DUT bug) -- a functional/simulation-time defect,
// invisible to this compile-only object model (same reasoning as the
// other "-fail" files in this test suite). Every structural assertion
// below is therefore identical to the passing file's by design.
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
class AssumePropertyUvmFailTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.14--assume-property-uvm-fail.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to AssumePropertyUvmFailTest go here!

TEST_F(AssumePropertyUvmFailTest, ModuleMemCtrlAndTopAndClassEnvExist) {
...
TEST_F(AssumePropertyUvmFailTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(AssumePropertyUvmFailTest, AssumePropertyIsReachable) {
...
TEST_F(AssumePropertyUvmFailTest, PropertySpecHasPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  ...
  const hldb::PropertySpec *const spec = assumeProp->getProperty<hldb::PropertySpec>();
...
TEST_F(AssumePropertyUvmFailTest, PropertyExprIsNegatedBitwiseAndOfReadAndWrite) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Assume *const assumeProp = any_cast<hldb::Assume>(top->getConcurrentAssertions()->front());
  ASSERT_NE(assumeProp, nullptr);
  ...
  const hldb::Operation *const notOp = spec->getPropertyExpr<hldb::Operation>();
  ...
  EXPECT_TRUE(OperandsContainNamedRef(andOp, std::string_view("dif.read"))) << "'dif.read' should be an operand of '&'";
  EXPECT_TRUE(OperandsContainNamedRef(andOp, std::string_view("dif.write"))) << "'dif.write' should be an operand of '&'";
}
