...
// tests/Google/chapter-16/16.12--property-prec-uvm.sv
// ----------------------------------------------------------------------------
...
// ============================================================================
//
// IEEE 1800-2023 construct under test (Sec 16.9.2, "Overlapped and
// non-overlapped implication"): `assert property (@(posedge dif.clk)
// dif.read |=> dif.write) else ...;` -- uses the NON-overlapped
// implication operator "|=>" (the consequent is checked starting the
// cycle *after* the antecedent holds), unlike test_16.12--property-prec.sv
// and test_16.10--property-local-var.cpp, which both use the overlapped
// "|->".
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
class PropertyPrecUvmTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.12--property-prec-uvm.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to PropertyPrecUvmTest go here!

TEST_F(PropertyPrecUvmTest, ModuleMemCtrlAndTopAndClassEnvExist) {
...
TEST_F(PropertyPrecUvmTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(PropertyPrecUvmTest, AssertPropertyIsReachable) {
...
TEST_F(PropertyPrecUvmTest, PropertySpecHasPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PropertyPrecUvmTest, PropertyExprIsNonOverlappedImplicationOfReadAndWrite) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Assert *const assertProp = any_cast<hldb::Assert>(top->getConcurrentAssertions()->front());
  ASSERT_NE(assertProp, nullptr);
