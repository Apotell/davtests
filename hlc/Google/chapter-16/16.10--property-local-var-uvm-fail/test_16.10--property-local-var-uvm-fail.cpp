...
// tests/Google/chapter-16/16.10--property-local-var-uvm-fail.sv
// ----------------------------------------------------------------------------
...
// test_16.10--property-local-var-uvm.cpp, except the consequent checks
// "dif.out == x + 3" instead of "dif.out == x + 4", and the else-clause
// calls `uvm_error instead of `uvm_info. Per the source's own
...
//     a Constant "3" (not "4" -- matching this file's mismatched addend).
//   - `assert property (prop) else `uvm_error(...);` is reachable via
...
class PropertyLocalVarUvmFailTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.10--property-local-var-uvm-fail.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to PropertyLocalVarUvmFailTest go here!

TEST_F(PropertyLocalVarUvmFailTest, ModuleClkGenAndTopAndClassEnvExist) {
...
TEST_F(PropertyLocalVarUvmFailTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(PropertyLocalVarUvmFailTest, PropertyPropDeclaredWithLocalVariableX) {
...
TEST_F(PropertyLocalVarUvmFailTest, PropertySpecHasPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::PropertyDecl *const prop = hldb::findByName<hldb::PropertyDecl>("prop", top->getPropertyDecls());
...
TEST_F(PropertyLocalVarUvmFailTest, PropertyExprIsOverlappedImplication) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::PropertyDecl *const prop = hldb::findByName<hldb::PropertyDecl>("prop", top->getPropertyDecls());
  ASSERT_NE(prop, nullptr);
  ...
  ASSERT_NE(prop->getPropertySpec()->getPropertyExpr(), nullptr)
      << "'(dif.valid, x = dif.in) |-> ##4 (dif.out == x + 3)' is the property expression";
...
TEST_F(PropertyLocalVarUvmFailTest, ConsequentIsCycleDelayedEqualityWithLocalVariable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::PropertyDecl *const prop = hldb::findByName<hldb::PropertyDecl>("prop", top->getPropertyDecls());
  ASSERT_NE(prop, nullptr);
  ...
  ASSERT_NE(delayOp, nullptr) << "'##4 (dif.out == x + 3)' should contain a vpiUnaryCycleDelayOp Operation";
  ...
  ASSERT_NE(eqOp, nullptr) << "'dif.out == x + 3' should contain a vpiEqOp Operation";
  EXPECT_TRUE(OperandsContainNamedRef(eqOp, std::string_view("dif.out"))) << "'dif.out' should be an operand of '=='";
  ...
  EXPECT_TRUE(OperandsContainNamedRef(addOp, std::string_view("x"))) << "'x' (the local variable) should be an operand of '+'";
  EXPECT_TRUE(OperandsContainConstant(addOp, std::string_view("3"))) << "the '+3' addend should be present (mismatched with "
                                                       "the DUT's actual +4, per the source's "
                                                       ":should_fail_because: comment)";
}

TEST_F(PropertyLocalVarUvmFailTest, AssertPropertyIsReachableAndReferencesProp) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);

  ASSERT_NE(top->getConcurrentAssertions(), nullptr)
      << "'assert property (prop) else `uvm_error(...);' should be reachable via "
