...
// tests/Google/chapter-16/16.9--sequence-fell-uvm.sv
// ----------------------------------------------------------------------------
...
//     bit clk_reg = 0;
//     assign out = clk_reg;
//     initial begin cnt = 0; clk_reg = 1; end
...
// functions" -- $rose/$fell/$stable/$past/$changed): this file is the
// "$fell" counterpart of test_16.9--sequence-changed-uvm.cpp -- same
// ClockedSeq/posedge-clocking-event shape, with $fell(dif.out) as the
// sequence body instead of $changed(dif.out). $fell(expr) is true when
// expr's least significant bit transitioned to 0 on the most recent
// clock tick.
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
//   - getSequenceExpr() is a SysFuncCall named "$fell" with exactly one
...
//   - Runtime pass/fail behavior of the assertion -- whether $fell(...)
//     actually evaluates true -- cannot be observed: HLC is a
...
class SequenceFellUvmTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.9--sequence-fell-uvm.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to SequenceFellUvmTest go here!

TEST_F(SequenceFellUvmTest, ModuleClkGenAndTopAndClassEnvExist) {
...
TEST_F(SequenceFellUvmTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(SequenceFellUvmTest, SequenceSeqDeclaredInModuleTop) {
...
TEST_F(SequenceFellUvmTest, SequenceExprIsClockedSeqWithPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::SequenceDecl *const seq = hldb::findByName<hldb::SequenceDecl>("seq", top->getSequenceDecls());
...
TEST_F(SequenceFellUvmTest, SequenceExprIsFellSysFuncCallOnOut) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::SequenceDecl *const seq = hldb::findByName<hldb::SequenceDecl>("seq", top->getSequenceDecls());
  ASSERT_NE(seq, nullptr);
  ...
  ASSERT_NE(clocked->getSequenceExpr(), nullptr) << "'$fell(dif.out)' is the sequence body";
  const hldb::SysFuncCall *const fell = any_cast<hldb::SysFuncCall>(clocked->getSequenceExpr());
  ASSERT_NE(fell, nullptr) << "'$fell(...)' returns a value, so this should be a SysFuncCall";
  EXPECT_EQ(fell->getName(), "$fell");

  ASSERT_NE(fell->getArguments(), nullptr);
  ASSERT_EQ(fell->getArguments()->size(), 1u) << "'$fell()' takes exactly one argument";
  EXPECT_TRUE(ArgumentsContainNamedRef(fell, std::string_view("dif.out"))) << "'dif.out' should be the argument";
}

TEST_F(SequenceFellUvmTest, AssertPropertyIsReachableAndReferencesSeq) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
