...
TEST_F(SequencePastUvmTest, SequenceExprIsBitwiseAndOfNegatedPastAndOut) {
  ...
  EXPECT_TRUE(OperandsContainNamedRef(andOp, "dif.out")) << "'dif.out' should directly be one operand of '&'";
  ...
  EXPECT_TRUE(ArgumentsContainNamedRef(past, std::string_view("dif.out"))) << "'dif.out' should be the argument to $past";
}
