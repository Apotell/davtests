...
// tests/Google/chapter-16/16.12--property-interface-uvm-fail.sv
// ----------------------------------------------------------------------------
...
// test_16.12--property-interface-uvm.cpp -- the same
// "assert property (@(posedge clk) !(read & write)) else ...;" declared
// inside the "mem_ctrl_if" interface. Per the source's own
...
// not in the interface/property declaration) -- a functional/simulation-
// time defect, invisible to this compile-only object model (same
// reasoning as the other "-fail" files in this test suite). Every
// structural assertion below is therefore identical to the passing file's
...
class PropertyInterfaceUvmFailTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.12--property-interface-uvm-fail.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to PropertyInterfaceUvmFailTest go here!

TEST_F(PropertyInterfaceUvmFailTest, ModuleMemCtrlAndTopAndClassEnvExist) {
...
TEST_F(PropertyInterfaceUvmFailTest, InterfaceMemCtrlIfExistsWithClkReadWritePorts) {
...
TEST_F(PropertyInterfaceUvmFailTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(PropertyInterfaceUvmFailTest, AssertPropertyIsReachableOnInterface) {
  const hldb::Interface *const iface = hldb::findByName<hldb::Interface>("mem_ctrl_if", m_design->getAllInterfaces());
...
TEST_F(PropertyInterfaceUvmFailTest, PropertySpecHasPosedgeClockingEvent) {
  const hldb::Interface *const iface = hldb::findByName<hldb::Interface>("mem_ctrl_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
...
TEST_F(PropertyInterfaceUvmFailTest, PropertyExprIsNegatedBitwiseAndOfReadAndWrite) {
  const hldb::Interface *const iface = hldb::findByName<hldb::Interface>("mem_ctrl_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
  const hldb::Assert *const assertProp = any_cast<hldb::Assert>(iface->getConcurrentAssertions()->front());
  ASSERT_NE(assertProp, nullptr);
