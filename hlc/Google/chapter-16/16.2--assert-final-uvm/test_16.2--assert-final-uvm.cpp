...
// tests/Google/chapter-16/16.2--assert-final-uvm.sv
// ----------------------------------------------------------------------------
...
//   - `assert final (...) else ...;` (in run_phase) -- this file's focus: a
//                             "final" deferred immediate assertion (Sec
//                             16.4), which defers evaluation to the very end
//                             of simulation. Written inside an explicit
//                             begin/end block nested in the task, with an
//                             else-clause but no explicit pass action.
//
// This is the "final" counterpart of test_16.2--assert0-uvm.cpp: identical
// UVM scaffolding (uvm_env subclass, phases, virtual interface), differing
// only in which deferred form of "assert" the task body under test uses.
// The surrounding UVM machinery is not itself IEEE 1800 assertion syntax and
// is out of scope here -- see NOT CHECKED below.
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
//     and getIsFinal() == true -- the "final" deferred form, distinct from
//     both the plain form above and from "assert #0" (Sec 16.4).
...
//   - Runtime pass/fail behavior of the assertion, and the specific
//     end-of-simulation timing that distinguishes "final" from "#0",
...
class AssertFinalUvmTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.2--assert-final-uvm.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to AssertFinalUvmTest go here!

TEST_F(AssertFinalUvmTest, ClassEnvExists) {
...
TEST_F(AssertFinalUvmTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
  const hldb::ClassDefn *const env = hldb::findByName<hldb::ClassDefn>("env", m_design->getAllClasses());
...
TEST_F(AssertFinalUvmTest, ConnectPhaseContainsPlainImmediateAssert) {
  const hldb::ClassDefn *const env = hldb::findByName<hldb::ClassDefn>("env", m_design->getAllClasses());
  ASSERT_NE(env, nullptr);
  const hldb::TaskFunc *const connectPhase = hldb::findByName<hldb::TaskFunc>("connect_phase", env->getMethods());
...
TEST_F(AssertFinalUvmTest, RunPhaseContainsFinalDeferredImmediateAssert) {
  ...
  ASSERT_NE(assertStmt, nullptr) << "'assert final (m_if.a != m_if.b) else ...;' should be an "
                                     "ImmediateAssert somewhere in run_phase's body";
...
TEST_F(AssertFinalUvmTest, RunPhaseAssertConditionIsNotEqualComparison) {
  const hldb::ClassDefn *const env = hldb::findByName<hldb::ClassDefn>("env", m_design->getAllClasses());
  ASSERT_NE(env, nullptr);
  const hldb::TaskFunc *const runPhase = hldb::findByName<hldb::TaskFunc>("run_phase", env->getMethods());
  ASSERT_NE(runPhase, nullptr);
