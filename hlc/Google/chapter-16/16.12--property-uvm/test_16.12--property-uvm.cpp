...
// tests/Google/chapter-16/16.12--property-uvm.sv
// ----------------------------------------------------------------------------
...
// ============================================================================
//
// IEEE 1800-2023 constructs under test: `assert property (@(posedge dif.clk)
// !(dif.read & dif.write)) else ...;` -- a property_spec written inline as
// a module item in module top (not inside a separately declared
// "property"), with the property expression itself a plain boolean
// expression: logical negation ("!") of a bitwise AND ("&"). This
// exercises the same PropertySpec/clocking-event shape as the plain
// property files in this batch, embedded in a UVM testbench the same way
// as the other chapter-16 UVM files.
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
class PropertyUvmTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.12--property-uvm.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to PropertyUvmTest go here!

TEST_F(PropertyUvmTest, ModuleMemCtrlAndTopAndClassEnvExist) {
...
TEST_F(PropertyUvmTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(PropertyUvmTest, AssertPropertyIsReachable) {
...
TEST_F(PropertyUvmTest, PropertySpecHasPosedgeClockingEvent) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(PropertyUvmTest, PropertyExprIsNegatedBitwiseAndOfReadAndWrite) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Assert *const assertProp = any_cast<hldb::Assert>(top->getConcurrentAssertions()->front());
  ASSERT_NE(assertProp, nullptr);
