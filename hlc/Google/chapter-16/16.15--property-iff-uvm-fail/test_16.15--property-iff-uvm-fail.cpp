...
// tests/Google/chapter-16/16.15--property-iff-uvm-fail.sv
// ----------------------------------------------------------------------------
...
// ============================================================================
//
// IMPORTANT NOTE on this file's source: as given, this file's "property
// prop" declaration ("@(posedge dif.clk) disable iff (dif.rst) dif.out;")
// is byte-for-byte IDENTICAL to test_16.15--property-iff-uvm.cpp's --
// there is no visible "wrong reset polarity" difference in the SV text
// itself, despite the source's own :should_fail_because: comment claiming
// one (compare to test_16.15--property-disable-iff-fail.cpp, where the
// polarity difference IS visible as "disable iff (~rst)" vs.
// "disable iff (rst)"). This looks like a duplication/labeling quirk in
// the test corpus rather than something to "fix" here -- these tests
// describe the source exactly as given, per the test-writing guide's
// instruction to assert against IEEE 1800-2023 semantics, not to second-
// guess or alter the source file. If the intended polarity difference is
// restored later, only the getDisableCondition() check below would need
// updating (to expect an Operation, as in the -disable-iff-fail file).
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
...
//     claimed polarity bug -- see note above).
...
class PropertyIffUvmFailTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.15--property-iff-uvm-fail.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to PropertyIffUvmFailTest go here!

TEST_F(PropertyIffUvmFailTest, ModuleClkGenAndTopAndClassEnvExist) {
...
TEST_F(PropertyIffUvmFailTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(PropertyIffUvmFailTest, PropertyPropDeclaredWithPosedgeClockingEventAndDisableCondition) {
