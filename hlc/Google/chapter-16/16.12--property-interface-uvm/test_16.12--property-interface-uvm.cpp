...
// tests/Google/chapter-16/16.12--property-interface-uvm.sv
// ----------------------------------------------------------------------------
...
// ============================================================================
//
// IEEE 1800-2023 constructs under test (Sec 16.12, "Declaring properties";
// Sec 25, "Interfaces"): `assert property (@(posedge clk)
// !(read & write)) else ...;` written directly inside an `interface`
// body, referencing the interface's own ports ("clk"/"read"/"write") --
// not a module. Per Sec 25.4, an interface is itself a kind of instance
// and may directly contain concurrent assertion items exactly like a
// module can. This is the same property-expression shape as
// test_16.12--property-uvm.cpp ("!(read & write)"), but declared in the
// interface instead of module top.
//
// ----------------------------------------------------------------------------
// CHECKED (this file):
//   - module "mem_ctrl", module "top", and class "env" all exist; "env"
//     has "connect_phase" (a Function) and "run_phase" (a Task).
//   - interface "mem_ctrl_if" exists (Design::getAllInterfaces()), and its
//     port list contains "clk", "read", and "write".
//   - `assert property (...) else ...;` is reachable via
//     Scope::getConcurrentAssertions() on the INTERFACE object (Interface
//     extends Instance, which is where getConcurrentAssertions() is
//     declared, per the same inheritance module uses), is an Assert, with
...
class PropertyInterfaceUvmTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "16.12--property-interface-uvm.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};
// ... All tests belonging to PropertyInterfaceUvmTest go here!

TEST_F(PropertyInterfaceUvmTest, ModuleMemCtrlAndTopAndClassEnvExist) {
...
TEST_F(PropertyInterfaceUvmTest, InterfaceMemCtrlIfExistsWithClkReadWritePorts) {
...
TEST_F(PropertyInterfaceUvmTest, ClassEnvHasConnectPhaseFunctionAndRunPhaseTask) {
...
TEST_F(PropertyInterfaceUvmTest, AssertPropertyIsReachableOnInterface) {
  const hldb::Interface *const iface = hldb::findByName<hldb::Interface>("mem_ctrl_if", m_design->getAllInterfaces());
...
TEST_F(PropertyInterfaceUvmTest, PropertySpecHasPosedgeClockingEvent) {
  const hldb::Interface *const iface = hldb::findByName<hldb::Interface>("mem_ctrl_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
...
TEST_F(PropertyInterfaceUvmTest, PropertyExprIsNegatedBitwiseAndOfReadAndWrite) {
  const hldb::Interface *const iface = hldb::findByName<hldb::Interface>("mem_ctrl_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
  const hldb::Assert *const assertProp = any_cast<hldb::Assert>(iface->getConcurrentAssertions()->front());
  ASSERT_NE(assertProp, nullptr);
  ...
  ASSERT_NE(andOp, nullptr) << "'read & write' should contain a vpiBitAndOp Operation";
  EXPECT_TRUE(OperandsContainNamedRef(andOp, "read")) << "'read' should be an operand of '&'";
  EXPECT_TRUE(OperandsContainNamedRef(andOp, "write")) << "'write' should be an operand of '&'";
}
