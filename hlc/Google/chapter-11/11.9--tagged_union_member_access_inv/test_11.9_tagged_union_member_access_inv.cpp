...
//
// Checked:
//   - module top has exactly 3 nets: "a", "b" ("u_int") and "c" (int),
//     none decl-assigned
...
//     RefObj "a" (resolving to Net "a") and RefObj "Valid" (resolving to
//     the union's TypespecMember "Valid", not a variable) -- i.e. the
//     HierPath references net "a", which was actually tagged Invalid, not
//     "b", which was actually tagged Valid
...
//     the source's ":should_fail_because:" annotation calls for. HLC is a
...
//     union_typespec.h, typespec_member.h, hier_path.h, net.h) -- there is
//     no field to assert against for "was this read invalid". The nearest
...
TEST_F(TaggedUnionMemberAccessInvTest, ModuleHasThreeNetsNoneDeclAssigned) {
...
TEST_F(TaggedUnionMemberAccessInvTest, ThirdStatementReadsAdotValidEvenThoughAIsTaggedInvalid) {
  ...
  EXPECT_EQ(path->getName(), "a.Valid") << "the HierPath references net 'a', which was tagged Invalid, not 'b'";
...
TEST_F(TaggedUnionMemberAccessInvTest, ReadingInactiveTagShouldRaiseARuntimeError) {
  GTEST_SKIP() << "The source is annotated ':should_fail_because: accessing "
                  ...
                  "(confirmed against net.h, tagged_pattern.h, hier_path.h) "
                  "-- there is nothing to assert against for 'this read was "
                  "invalid' today. If runtime tag-checking is ever added, "
