...
// Tests for 11.5.1--simple_idx_neg_part_select-sim.sv (tags: 11.5.1)
...
//     vpiIndexedPartSelectType=neg indexed, vpiBaseExpr Constant "15",
...
class SimpleIdxNegPartSelectSimTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "11.5.1--simple_idx_neg_part_select-sim.hlc"}); }
...
TEST_F(SimpleIdxNegPartSelectSimTest, ModuleExists) { EXPECT_NE(getTop(), nullptr); }

TEST_F(SimpleIdxNegPartSelectSimTest, ModuleHasTwoNetsAllWire) {
...
TEST_F(SimpleIdxNegPartSelectSimTest, ModuleHasNoVariables) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
// --- continuous assignment: indexed negative part-select ----

TEST_F(SimpleIdxNegPartSelectSimTest, ContAssignIsANegIndexedFifteenMinusFour) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(SimpleIdxNegPartSelectSimTest, DesignHasTwoTypespecs) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_EQ(m_design->getTypespecs()->size(), 2u);
}

TEST_F(SimpleIdxNegPartSelectSimTest, CompilerReportsZeroErrors) {
...
TEST_F(SimpleIdxNegPartSelectSimTest, NoProcesses) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
