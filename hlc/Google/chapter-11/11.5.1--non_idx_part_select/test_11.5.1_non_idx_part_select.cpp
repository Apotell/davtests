...
// Tests for 11.5.1--non_idx_part_select.sv (tags: 11.5.1)
//   logic [15:0] a;
//   logic [3:0] b;
//
//   initial begin
...
//   end
//
// Per IEEE 1800-2017 11.5.1, "a[msb:lsb]" is the *non-indexed* (constant
// range) part-select: both bounds are given directly, unlike the
// "+:"/"-:" indexed-part-select siblings of this file which give a single
// base plus a width. This is a procedural select (inside initial), unlike
...
class NonIdxPartSelectTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "11.5.1--non_idx_part_select.hlc"}); }
...
TEST_F(NonIdxPartSelectTest, ModuleHasNoContinuousAssignments) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(NonIdxPartSelectTest, InitialBlockHasOneBlockingAssignment) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(NonIdxPartSelectTest, AssignmentRhsIsElevenToEightPartSelect) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = any_cast<hldb::Initial>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
...
TEST_F(NonIdxPartSelectTest, DesignHasTwoTypespecs) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_EQ(m_design->getTypespecs()->size(), 2u);
}

TEST_F(NonIdxPartSelectTest, CompilerReportsZeroErrors) {
