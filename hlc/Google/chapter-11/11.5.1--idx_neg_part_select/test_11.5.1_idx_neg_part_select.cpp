...
// Tests for 11.5.1--idx_neg_part_select.sv (tags: 11.5.1)
//   logic [15:0] a;
//   logic [3:0] b;
//
//   initial begin
...
//   end
//
// Per IEEE 1800-2017 11.5.1, "a[base -: width]" is the *indexed negative*
// part-select: the selected range grows downward from base, i.e. bits
// [base -: width] == [base : base-width+1]. This is a procedural select
// (inside initial), unlike the "simple_idx_neg_part_select" continuous-
// assignment sibling of this file, and it must be distinguished from the
// "+:"/pos-indexed sibling of *this* file: same base/width literals (11, 4)
// but the opposite direction and vpiIndexedPartSelectType.
//
// Checked:
...
//     Variable "a", vpiIndexedPartSelectType == vpiNegIndexed (not
//     vpiPosIndexed -- the direction the "-:" spelling implies),
//     vpiBaseExpr Constant "11", vpiWidthExpr Constant "4"
...
class IdxNegPartSelectTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "11.5.1--idx_neg_part_select.hlc"}); }
...
TEST_F(IdxNegPartSelectTest, ModuleExists) { EXPECT_NE(getTop(), nullptr); }

TEST_F(IdxNegPartSelectTest, ModuleHasTwoVectorVariablesNeitherDeclAssigned) {
...
TEST_F(IdxNegPartSelectTest, ModuleHasNoContinuousAssignments) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(IdxNegPartSelectTest, InitialBlockHasOneBlockingAssignment) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(IdxNegPartSelectTest, AssignmentRhsIsNegIndexedElevenMinusFour) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = any_cast<hldb::Initial>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
...
TEST_F(IdxNegPartSelectTest, DesignHasTwoTypespecs) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_EQ(m_design->getTypespecs()->size(), 2u);
}

TEST_F(IdxNegPartSelectTest, CompilerReportsZeroErrors) {
