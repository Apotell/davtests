...
TEST_F(AssignInExpSimTest, ModuleHasThreeIntVariables) {
  const hldb::Module *const top = getTop();
...
TEST_F(AssignInExpSimTest, ModuleHasNoNets) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(AssignInExpSimTest, BEndsUpEqualToCMinusOne) {
  GTEST_SKIP() << "The source asserts b == c - 1 after 'c = a; b = (a -= 1);' runs -- i.e. that "
                  ...
                  "Variable (only a declaration-time getValue<T>(), which none of a/b/c have here). "
  ...
  // initializer; 'b' has none (it is assigned inside the initial block),
