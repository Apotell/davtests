...
// Tests for 11.5.1--idx_neg_part_select-sim.sv (tags: 11.5.1)
...
//   end
//
// Per IEEE 1800-2017 11.5.1: a[15-:8] selects the high 8 bits of
// a = 16'h1234, growing downward from base 15, i.e. bits [15:8] = 8'h12.
//
// Checked:
//   - module top has exactly 2 nets: "a" (LogicTypespec, vector [15:0],
//     decl-assigned Constant "16'h1234") and "b" (LogicTypespec, vector
//     [7:0], not decl-assigned)
...
//   - the Assignment: lhs RefObj "b"; rhs IndexedPartSelect "a[15-:8]"
//     whose vpiPrefix RefObj "a" resolves to Net "a",
//     vpiIndexedPartSelectType == vpiNegIndexed, vpiBaseExpr Constant "15",
//     vpiWidthExpr Constant "8"
//   - the SysTaskCall "$display" has 2 arguments: Constant string
//     ":assert: (0x12 == 0x%x)" and RefObj "b"
...
//   - Whether b actually equals 8'h12 at runtime. HLC is a compiler/
...
class IdxNegPartSelectSimTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "11.5.1--idx_neg_part_select-sim.hlc"}); }
...
TEST_F(IdxNegPartSelectSimTest, ModuleExists) { EXPECT_NE(getTop(), nullptr); }

TEST_F(IdxNegPartSelectSimTest, ModuleHasTwoNetsOnlyADeclAssigned) {
...
// --- initial block: procedural negative indexed part-select -----------------

TEST_F(IdxNegPartSelectSimTest, InitialBlockHasTwoStatements) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(IdxNegPartSelectSimTest, AssignmentRhsIsNegIndexedFifteenMinusEight) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = any_cast<hldb::Initial>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
  ...
  EXPECT_EQ(sel->getName(), "a[15-:8]");
  EXPECT_EQ(sel->getPrefix<hldb::RefObj>()->getName(), "a");
  EXPECT_EQ(sel->getIndexedPartSelectType(), vpiNegIndexed);
  ASSERT_NE(sel->getBaseExpr<hldb::Constant>(), nullptr);
  EXPECT_EQ(sel->getBaseExpr<hldb::Constant>()->getDecompile(), "15");
  ASSERT_NE(sel->getWidthExpr<hldb::Constant>(), nullptr);
  EXPECT_EQ(sel->getWidthExpr<hldb::Constant>()->getDecompile(), "8");
}

TEST_F(IdxNegPartSelectSimTest, SecondStatementDisplaysExpectedBValue) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = any_cast<hldb::Initial>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
  ...
  EXPECT_EQ(any_cast<hldb::Constant>(disp->getArguments()->at(0))->getValue(), ":assert: (0x12 == 0x%x)");
  EXPECT_EQ(any_cast<hldb::RefObj>(disp->getArguments()->at(1))->getName(), "b");
}

// --- design-level typespecs / compiler diagnostics --------------------------

TEST_F(IdxNegPartSelectSimTest, DesignHasFourTypespecs) {
  ASSERT_NE(m_design->getTypespecs(), nullptr);
  EXPECT_EQ(m_design->getTypespecs()->size(), 4u);
}

TEST_F(IdxNegPartSelectSimTest, CompilerReportsZeroErrors) {
...
TEST_F(IdxNegPartSelectSimTest, BEqualsHex12AtRuntime) {
  GTEST_SKIP() << "IEEE 1800-2017 11.5.1: a = 16'h1234, so a[15-:8] == 8'h12, "
                  "per the ':assert:' tag authored into the source. HLC is a "
                  "static compiler/elaborator: Net::getValue<T>() only ever "
                  "exposes a declaration-time initializer, and 'b' is only "
  ...
  EXPECT_EQ(bValue->getDecompile(), "8'h12");
}
