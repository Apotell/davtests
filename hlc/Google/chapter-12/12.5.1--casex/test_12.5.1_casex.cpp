...
// Tests for 12.5.1--casex.sv (tags: 12.5.1)
//   module case_tb ();
//     wire [3:0] a = 4'b10zx;
//     reg [3:0] b = 0;
//     always @* begin
...
//   casex". "casex" treats both z and x bits as do-not-care during
//   comparison. "The default statement shall be optional... vpi_iterate()
//   shall return NULL for the default case item because there is no
//   expression with the default case" (Annex 37.72). This file has 4
...
//   file's "casex" keyword maps to vpiCaseX.
//
// What is checked:
//   - module case_tb has exactly 1 Net "a" (wire) and 1 Variable "b" (reg)
//   - CaseStmt exists, getCaseType() == vpiCaseX, getQualifier() ==
...
//     with the exact source literal decompiled text ("4'b1xz?",
//     "4'b01xx", "4'b001x", "4'b0001"), and a body Assignment to
//     Variable "b" with the corresponding value (1, 2, 3, 4)
//   - the 5th (default) CaseItem has a null getExprs() (per Annex
//     37.72 detail 2: "vpi_iterate() shall return NULL for the default
//     case item") and body Assignment "b = 0"
//   - no continuous assignments exist in the module
//
// What is NOT checked and why:
//   - the runtime do-not-care matching behavior itself (whether x/z
//     bits in "a" actually match during simulation) is a simulation-
//     time concept, not a static/structural compile-time property
...
class CasexTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "12.5.1--casex.hlc"}); }
...
TEST_F(CasexTest, ModuleExists) { EXPECT_NE(getTop(), nullptr); }

TEST_F(CasexTest, ModuleHasOneNetAAndOneVariableB) {
  const hldb::Module *const top = getTop();
...
TEST_F(CasexTest, CaseStmtExistsWithCaseXTypeAndNoQualifier) {
...
TEST_F(CasexTest, ConditionResolvesToNetANotVariable) {
  const hldb::CaseStmt *const cs = getCaseStmt();
...
TEST_F(CasexTest, ExactlyFiveCaseItems) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
...
TEST_F(CasexTest, FirstItemIs1xzQAssigningOne) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
  ASSERT_GE(cs->getCaseItems()->size(), 1u);
  checkExplicitItem(cs->getCaseItems()->at(0), "4'b1xz?", "1");
}

TEST_F(CasexTest, SecondItemIs01xxAssigningTwo) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
  ASSERT_GE(cs->getCaseItems()->size(), 2u);
  checkExplicitItem(cs->getCaseItems()->at(1), "4'b01xx", "2");
}

TEST_F(CasexTest, ThirdItemIs001xAssigningThree) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
  ASSERT_GE(cs->getCaseItems()->size(), 3u);
  checkExplicitItem(cs->getCaseItems()->at(2), "4'b001x", "3");
}

TEST_F(CasexTest, FourthItemIs0001AssigningFour) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
...
TEST_F(CasexTest, FifthItemIsDefaultWithNullExprsAssigningZero) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
...
TEST_F(CasexTest, NoContinuousAssigns) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
