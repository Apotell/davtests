...
// Tests for 12.6.1--casez_pattern.sv (tags: 12.6.1)
//   module case_tb ();
...
//   shapes, same $display bodies) except the case-keyword is "casez"
//   and the constant-expression sub-patterns use z/"?" wildcard
//   literals ("4'bzz0?", "4'hz00?") instead of plain values.
//
//   IMPORTANT source-file note (same as 12.6.1--case_pattern.sv):
//   "casez (v)" reads the identifier "v", which is declared nowhere in
...
//
//   Per IEEE 1800-2023 vpi_user.h vpiCaseType enumerants (vpiCaseZ=3
//   for "casez", confirmed against this repo's actual
...
//   "inside" in 12.5.4--case_set.cpp), this file's "casez ... matches"
//   maps to getCaseType() == vpiCaseZ and getQualifier() ==
//   vpiMatchesQualifier.
//
...
//   12.6.1--case_pattern.cpp and 12.6.1--casex_pattern.cpp): (1)
//   getQualifier() comes back 0 instead of vpiMatchesQualifier; (2) the
//   undeclared case_expression "v" produces zero diagnostics instead of
//   COMP_UNDEFINED_VARIABLE; (3) the pattern-bound "v" referenced in case
//   items 1 and 4's $display never resolves via
//   getActual<hldb::AnyPattern>(). All three kept as GTEST_SKIP() with
//   the real assertions underneath, per the established gating rule
//   (skips only added after personal verification).
//
// What is checked:
//   - module case_tb has zero Nets and exactly 1 Variable "tmp"
//   - CaseStmt exists, getCaseType() == vpiCaseZ; its qualifier SHOULD
...
//     the source, including the z/"?" wildcard literals
...
//     actually match during simulation, and that x bits would NOT be
//     treated as do-not-care the way casex would) is a simulation-time
//     concept, not a static/structural property
...
class CasezPatternTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "12.6.1--casez_pattern.hlc"}); }
...
TEST_F(CasezPatternTest, ModuleExists) { EXPECT_NE(getTop(), nullptr); }

TEST_F(CasezPatternTest, ModuleHasNoNetsAndOneVariableTmp) {
  const hldb::Module *const top = getTop();
...
TEST_F(CasezPatternTest, CaseStmtExistsWithCaseZTypeAndMatchesQualifier) {
  GTEST_SKIP() << "Confirmed HLC bug -- verified by running this test with the skip removed (fails as expected): "
                  ...
                  "gap as 12.6.1--case_pattern.cpp/casex_pattern.cpp and "
                  "12.4.2--priority_if.cpp/unique_if.cpp/unique0_if.cpp. Tracked, not yet fixed by the compiler.";
...
TEST_F(CasezPatternTest, CaseExpressionVIsUndeclaredAndFailsToBind) {
  GTEST_SKIP() << "Confirmed HLC bug -- verified by running this test with the skip removed (fails as expected): "
                  "'casez (v)' reads an identifier declared nowhere in this module, which IEEE 1800-2023 Sec 6.5 "
                  ...
                  "in src/. Same underlying gap as 12.6.1--case_pattern.cpp/casex_pattern.cpp and "
  ...
  // 12.6.1--casez_pattern.sv:31:17 -- the "v" inside "initial casez (v) matches".
  ASSERT_NE(findError(ErrorDefinition::COMP_UNDEFINED_VARIABLE, "v", 31, 17), nullptr)
      << "'casez (v)' references an identifier that is declared nowhere in this module (only 'tmp' of a "
...
TEST_F(CasezPatternTest, ExactlyFourCaseItems) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
...
TEST_F(CasezPatternTest, FirstItemIsTaggedAWithIdentifierVAndWildcardBinaryLiteral) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
  ASSERT_GE(cs->getCaseItems()->size(), 1u);
  checkItemPattern(cs->getCaseItems()->at(0), "a", SubKind::kIdentifier, "v", SubKind::kConstant, "4'bzz0?");
}

TEST_F(CasezPatternTest, FirstItemDisplayReferencesPatternBoundV) {
  GTEST_SKIP() << "Confirmed HLC bug -- verified by running this test with the skip removed (fails as expected): "
                  "the 'v' in this item's $display never resolves to the AnyPattern declared by 'tagged a '{.v, "
                  "4'bzz0?}' -- same pattern-identifier binding gap confirmed in "
                  "12.6.1--case_pattern.cpp/casex_pattern.cpp. Tracked, not yet fixed by the compiler.";
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
  ASSERT_GE(cs->getCaseItems()->size(), 1u);
...
TEST_F(CasezPatternTest, SecondItemIsTaggedAWithIdentifiersV1V2) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
...
TEST_F(CasezPatternTest, ThirdItemIsTaggedBWithIdentifiersV1V2) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
...
TEST_F(CasezPatternTest, FourthItemIsTaggedCWithWildcardHexLiteralAndIdentifierV) {
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
  ASSERT_GE(cs->getCaseItems()->size(), 4u);
  checkItemPattern(cs->getCaseItems()->at(3), "c", SubKind::kConstant, "4'hz00?", SubKind::kIdentifier, "v");
}

TEST_F(CasezPatternTest, FourthItemDisplayReferencesPatternBoundV) {
  GTEST_SKIP() << "Confirmed HLC bug -- verified by running this test with the skip removed (fails as expected): "
                  "same pattern-identifier binding gap as FirstItemDisplayReferencesPatternBoundV, confirmed here "
                  "for item 4's 'tagged c '{4'hz00?, .v}' pattern too. Tracked, not yet fixed by the compiler.";
  const hldb::CaseStmt *const cs = getCaseStmt();
  ASSERT_NE(cs, nullptr);
  ASSERT_NE(cs->getCaseItems(), nullptr);
  ASSERT_GE(cs->getCaseItems()->size(), 4u);
...
TEST_F(CasezPatternTest, NoContinuousAssigns) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
