...
// Spec-based validation of basic string literal usage per IEEE 1800-2017 Sec 5.9.
//
...
//     size      = 64   -- "one line" = 8 characters x 8 bits = 64 bits (correct)
//     value     = "one line"
//     typespec  = StringTypespec
//
// Note on size correctness:
//   Unlike Sec 5.9.1 escape-sequence strings (which Surelog stores verbatim and
...
// Module structure
// ----
TEST_F(StringBasics, ModuleExists) { ASSERT_NE(getTop(m_design), nullptr) << "module 'top' not found"; }

TEST_F(StringBasics, ModuleHasNoNets) {
  const hldb::Module *const m = getTop(m_design);
  ASSERT_NE(m, nullptr);
  // Sec 5.9: the module only contains an initial block -- no variable declarations.
...
// ----
// Sec 5.9: the string literal "one line" as a $display argument.
...
// ----
// Sec 5.9: size = 8 x number of characters.
// "one line" = 8 characters -> 8 x 8 = 64 bits.
// No escape sequences -- Surelog correctly computes the size (64).
// ----
TEST_F(StringBasics, Argument_SizeIs64PerSpec) {
  const hldb::Constant *const c = getStringArg(m_design);
  ASSERT_NE(c, nullptr);
  EXPECT_EQ(c->getSize(), 64) << "Sec 5.9: \"one line\" = 8 characters x 8 bits = 64 bits";
}

// ----
// Sec 5.9: the string value stored in UHDM must match the source characters.
// vpiValue returns the raw string content without surrounding double quotes.
// ----
TEST_F(StringBasics, Argument_ValueIsOneLine) {
  const hldb::Constant *const c = getStringArg(m_design);
  ASSERT_NE(c, nullptr);
  EXPECT_EQ(c->getValue(), "one line") << "Sec 5.9: string literal value must be \"one line\"";
}

// ----
// Sec 5.9: string literals carry a StringTypespec -- not LogicTypespec or any
