...
//   2. The number of bits required to hold a string is 8 x number of chars.
...
//   The LHS variables carry their declared typespecs (ByteTypespec, BitTypespec,
...
// Module structure
// ----
TEST_F(StringAssignment, ModuleExists) { ASSERT_NE(getTop(m_design), nullptr) << "module 'top' not found"; }

TEST_F(StringAssignment, ThreeVariablesExist) {
  const hldb::Module *const m = getTop(m_design);
  ASSERT_NE(m, nullptr);
  ASSERT_NE(m->getVariables(), nullptr);
  EXPECT_EQ(m->getVariables()->size(), 3u) << "expected 3 variables: a (byte), b (bit), c (logic)";
}

// None of byte/bit/logic is a net-type keyword, so per IEEE 1800-2023 Sec
// 6.7/6.8 none of a, b, c must appear in the module's net collection.
TEST_F(StringAssignment, ModuleHasNoNets) {
  const hldb::Module *const m = getTop(m_design);
  ASSERT_NE(m, nullptr);
  EXPECT_TRUE(!m->getNets() || m->getNets()->empty()) << "byte/bit/logic declarations must not appear as Nets";
}

// ----
// Sec 5.9: integral type declarations -- each type has a distinct UHDM typespec.
// byte -> ByteTypespec, bit [7:0] -> BitTypespec, logic [7:0] -> LogicTypespec.
// ----
TEST_F(StringAssignment, VariableA_HasByteTypespec) {
  const hldb::Variable *const var = getVariable(m_design, "a");
  ASSERT_NE(var, nullptr);
  ASSERT_NE(var->getTypespec(), nullptr) << "variable 'a' has no typespec";
  EXPECT_NE(var->getTypespec()->getActual<hldb::ByteTypespec>(), nullptr)
      << "Sec 5.9: 'byte a' must produce a ByteTypespec";
}

TEST_F(StringAssignment, VariableB_HasBitTypespec) {
  ...
  EXPECT_NE(var->getTypespec()->getActual<hldb::BitTypespec>(), nullptr)
      << "Sec 5.9: 'bit [7:0] b' must produce a BitTypespec";
}

TEST_F(StringAssignment, VariableC_HasLogicTypespec) {
  const hldb::Variable *const var = getVariable(m_design, "c");
  ASSERT_NE(var, nullptr);
  ASSERT_NE(var->getTypespec(), nullptr) << "variable 'c' has no typespec";
  EXPECT_NE(var->getTypespec()->getActual<hldb::LogicTypespec>(), nullptr)
      << "Sec 5.9: 'logic [7:0] c' must produce a LogicTypespec";
}

// ----
// Sec 5.9: 'byte' is an implicit 8-bit type -- no explicit packed dimension is
// needed in the source, so ByteTypespec carries no range in UHDM.
// This distinguishes 'byte a' from 'bit signed [7:0] a'.
// ----
TEST_F(StringAssignment, VariableA_ByteTypespec_HasNoExplicitRange) {
  const hldb::Variable *const var = getVariable(m_design, "a");
  ASSERT_NE(var, nullptr);
  ASSERT_NE(var->getTypespec(), nullptr);
  const auto *bt = var->getTypespec()->getActual<hldb::ByteTypespec>();
  ASSERT_NE(bt, nullptr);
  EXPECT_TRUE(!bt->getRanges() || bt->getRanges()->empty())
      << "Sec 5.9: 'byte' is an implicit 8-bit keyword type with no explicit "
         "packed dimension range in UHDM";
}

// ----
// Sec 5.9: 'bit [7:0]' has an explicit packed dimension -- BitTypespec must carry
// a [7:0] range.
// ----
TEST_F(StringAssignment, VariableB_RangeLeftIs7) {
  const hldb::Variable *const var = getVariable(m_design, "b");
  ASSERT_NE(var, nullptr);
  ASSERT_NE(var->getTypespec(), nullptr);
...
TEST_F(StringAssignment, VariableB_RangeRightIs0) {
  const hldb::Variable *const var = getVariable(m_design, "b");
  ASSERT_NE(var, nullptr);
  ASSERT_NE(var->getTypespec(), nullptr);
  const auto *bt = var->getTypespec()->getActual<hldb::BitTypespec>();
...
// ----
// Sec 5.9: 'logic [7:0]' has an explicit packed dimension -- LogicTypespec must
// carry a [7:0] range.
// ----
TEST_F(StringAssignment, VariableC_RangeLeftIs7) {
  const hldb::Variable *const var = getVariable(m_design, "c");
  ASSERT_NE(var, nullptr);
  ASSERT_NE(var->getTypespec(), nullptr);
  const auto *lt = var->getTypespec()->getActual<hldb::LogicTypespec>();
...
TEST_F(StringAssignment, VariableC_RangeRightIs0) {
  const hldb::Variable *const var = getVariable(m_design, "c");
  ASSERT_NE(var, nullptr);
  ASSERT_NE(var->getTypespec(), nullptr);
  const auto *lt = var->getTypespec()->getActual<hldb::LogicTypespec>();
...
// ----
// Sec 5.9: each RHS string literal must be vpiStringConst (6).
// The size must be 8 bits -- one character x 8 bits per character.
// ----
TEST_F(StringAssignment, AllRhsAreStringConstType) {
  for (std::size_t i = 0; i < 3; ++i) {
    ...
    EXPECT_EQ(c->getConstType(), 6) << "stmt[" << i << "]: Sec 5.9 string literal must be vpiStringConst (6)";
  }
}

TEST_F(StringAssignment, AllRhsHaveSize8) {
  // Sec 5.9: "The number of bits required to hold a string is 8 times the number
  ...
  for (std::size_t i = 0; i < 3; ++i) {
    ...
    const auto *c = assign->getRhs<hldb::Constant>();
    ASSERT_NE(c, nullptr) << "stmt[" << i << "] RHS is not a Constant";
    EXPECT_EQ(c->getSize(), 8) << "stmt[" << i << "]: Sec 5.9: 1-character string = 8 bits";
  }
}
...
TEST_F(StringAssignment, AllRhsHaveStringTypespec) {
  for (std::size_t i = 0; i < 3; ++i) {
    ...
    EXPECT_NE(c->getTypespec()->getActual<hldb::StringTypespec>(), nullptr)
        << "stmt[" << i << "]: Sec 5.9 string literal must have StringTypespec";
  }
}

// ----
// Sec 5.9: per-assignment value checks.
