...
//   tests/Google/chapter-21/21.3--fwrite-boh.sv   (:name: fwrite_boh,
...
//       declarations therefore differ in TYPE as well as in name.
...
//       Begin holding the four statements in source order. Nothing is
//       declared inside the block -- both variables are module-level -- so
//       the Begin owns no variables of its own and every name used inside it
//       resolves outward to the module scope.
...
//       named for. $fwriteb, $fwriteo and $fwriteh are three DISTINCT system
//       tasks in that clause, not one task carrying a radix modifier, and
//       every task in the clause takes the file descriptor as its FIRST
//       argument followed by the values to write -- the opposite of the
//       destination-first order of Sec 21.3.4.2's $fgets and Sec 21.3.4.4's
//       $fread. Being tasks, all three are SysTaskCalls, never SysFuncCalls.
//       Two further facts come straight from the source text rather than from
//       the clause in general:
//         * the file writes ONLY the suffixed forms. The plain "$fwrite" of
//           the sibling 21.3--fwrite.sv appears nowhere here, so no call in
//           this file may carry that name. (This fixture is the "-boh" half
//           of the pair; the plain half is tested separately.)
//         * each call's value argument is the identifier "str", not a
//           literal, so each must carry a REFERENCE to the declared string
//           variable rather than a fresh Constant holding "abc" folded in
//           from the declaration.
//       The three suffixes differ only in the default radix used for
//       arguments written without a format specification, which is a
//       formatting decision made while time advances -- see "WHAT IS NOT
//       CHECKED" below.
...
//     and holds exactly 4 statements.
...
//   - stmt[1], stmt[2] and stmt[3] are SysTaskCalls named, in source order,
//     "$fwriteb", "$fwriteo" and "$fwriteh" -- three distinct names, checked
//     one by one, which is the point of this file. None of them is a
//     SysFuncCall.
//   - the three names are all different from one another, and none of them is
//     the plain "$fwrite" that this file never writes.
//   - each of the three carries exactly 2 arguments: RefObj "fd" bound to
//     Variable "fd" first, then RefObj "str" bound to Variable "str" -- the
//     Sec 21.3.2 descriptor-first order -- and in each the value argument is
//     a reference, never a Constant.
//   - the three calls are three DISTINCT nodes, checked pairwise.
...
//   - The default output radix each suffix selects: binary for $fwriteb,
//     octal for $fwriteo and hexadecimal for $fwriteh. A radix is applied
//     when a value is formatted for output, which only happens while time
//     advances. The nearest real assertions are
//     SecondStatementIsFwriteB, ThirdStatementIsFwriteO and
//     FourthStatementIsFwriteH, backed by
//     TheThreeVariantsHaveThreeDistinctNames: together they pin that the
//     source really did name three distinct tasks in that order -- the
//     static half of the same concern.
//   - That $fwrite and its suffixed forms emit no trailing newline, the one
//     behavior separating them from the $fdisplay family (Sec 21.2.1). A
//     trailing newline is part of text produced while time advances. What the
//     source determines is which tasks were named, and that is asserted by
//     the same three per-variant tests.
//   - The characters actually written to the file. Formatting happens while
//     time advances. The nearest real assertion is
//     EachVariantTakesDescriptorThenTheStringVariable.
...
//   - Whether "str" still holds "abc" by the time the three calls read it.
...
//     four statements inside the initial run in the order written. Execution
//     order is a time-domain fact. The nearest real assertions are
//     ModuleHasInitialThenFinalProcess and
//     InitialBodyIsBeginWithFourStatements, which pin that the procedures and
...
//     InitialBodyIsBeginWithFourStatements, which requires the block to own
...
class FwriteBohTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "21.3--fwrite-boh.hlc"}); }
 ...
 protected:
  ...
  // Shared body for the three "$fwrite<suffix>(fd, str);" statements, which
  // differ only in name and position. Sec 21.3.2 gives them all the same
  // shape: a system task taking the descriptor first, then the values.
  static void checkFwriteVariant(uint32_t stmtIndex, const char *expectedName) {
    ...
    EXPECT_EQ(any_cast<hldb::SysFuncCall>(body->getStmts()->at(stmtIndex)), nullptr)
        << expectedName
        << " is a file output system task (IEEE 1800-2023 Sec 21.3.2), so it must not "
           "be a SysFuncCall";
    ...
    EXPECT_NE(call->getName(), "$fwrite")
        << "this file writes only the suffixed forms -- the plain '$fwrite' of 21.3--fwrite.sv is "
           "never written here";
...
TEST_F(FwriteBohTest, ModuleTopExists) { EXPECT_NE(getTop(), nullptr); }
...
TEST_F(FwriteBohTest, ModuleHasNoNetsNoPortsAndNoContinuousAssignments) {
  const hldb::Module *const top = getTop();
...
TEST_F(FwriteBohTest, ModuleHasExactlyTwoDistinctVariablesFdAndStr) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
// type.
TEST_F(FwriteBohTest, VariableFdIsSignedIntAndNotAString) {
  const hldb::Variable *const fd = getVariable("fd");
  ASSERT_NE(fd, nullptr);
...
TEST_F(FwriteBohTest, VariableStrIsAStringInitializedToAbc) {
  const hldb::Variable *const str = getVariable("str");
  ASSERT_NE(str, nullptr);
...
TEST_F(FwriteBohTest, ModuleHasInitialThenFinalProcess) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
// Sec 9.3.1: the "begin ... end" holds the four statements written. Nothing
// is declared inside it -- both "fd" and "str" are module-level here -- so
// the block owns no variables of its own.
TEST_F(FwriteBohTest, InitialBodyIsBeginWithFourStatements) {
  const hldb::Begin *const body = getInitialBody();
  ...
  ASSERT_NE(body->getStmts(), nullptr);
  EXPECT_EQ(body->getStmts()->size(), 4u) << "'fd = $fopen(...);' plus the three '$fwrite*' calls";
}

// --- stmt[0]: fd = $fopen("tmp.txt", "w"); -----------------------------------

TEST_F(FwriteBohTest, FirstStatementAssignsFopenResultToFd) {
  const hldb::Begin *const body = getInitialBody();
  ASSERT_NE(body, nullptr);
...
TEST_F(FwriteBohTest, FopenArgumentsAreFilenameAndMode) {
  const hldb::Begin *const body = getInitialBody();
  ASSERT_NE(body, nullptr);
  const hldb::Assignment *const assign = any_cast<hldb::Assignment>(body->getStmts()->at(0));
...
// --- stmt[1..3]: the three suffixed $fwrite tasks ----------------------------
//
// Sec 21.3.2 lists $fwriteb, $fwriteo and $fwriteh as three separate system
// tasks. Each is checked by name at its own source position, because "three
// distinct task names were written, in this order" is precisely what this
// fixture exists to pin down; a model that collapsed them into one task, or
// that reordered them, would pass a laxer check.

TEST_F(FwriteBohTest, SecondStatementIsFwriteB) { checkFwriteVariant(1u, "$fwriteb"); }

TEST_F(FwriteBohTest, ThirdStatementIsFwriteO) { checkFwriteVariant(2u, "$fwriteo"); }

TEST_F(FwriteBohTest, FourthStatementIsFwriteH) { checkFwriteVariant(3u, "$fwriteh"); }

// The three names must all differ from one another, and none may be the plain
// "$fwrite" the source never writes. Checking them as a set complements the
// positional checks above: it fails loudly if the model resolved every suffix
// back to one canonical name.
TEST_F(FwriteBohTest, TheThreeVariantsHaveThreeDistinctNames) {
  const hldb::Begin *const body = getInitialBody();
  ASSERT_NE(body, nullptr);
  ASSERT_NE(body->getStmts(), nullptr);
  ASSERT_EQ(body->getStmts()->size(), 4u);

  const hldb::SysTaskCall *calls[3] = {nullptr, nullptr, nullptr};
  for (uint32_t i = 0; i < 3u; ++i) {
    calls[i] = any_cast<hldb::SysTaskCall>(body->getStmts()->at(i + 1u));
    ASSERT_NE(calls[i], nullptr) << "stmt[" << (i + 1u) << "] should be a SysTaskCall";
    EXPECT_NE(calls[i]->getName(), "$fwrite")
        << "stmt[" << (i + 1u)
        << "]: this file writes only the suffixed forms, so the plain '$fwrite' must appear nowhere";
  }
  for (uint32_t i = 0; i < 3u; ++i) {
    for (uint32_t j = i + 1u; j < 3u; ++j) {
      EXPECT_NE(calls[i]->getName(), calls[j]->getName())
          << "IEEE 1800-2023 Sec 21.3.2: $fwriteb, $fwriteo and $fwriteh are three separate system "
             "tasks, not one task with a radix modifier -- stmt["
          << (i + 1u) << "] and stmt[" << (j + 1u) << "] must not share a name";
    }
  }
}

// Three separately written calls must survive as three separate nodes, and
// all three must name the same descriptor and the same string variable.
TEST_F(FwriteBohTest, EachVariantTakesDescriptorThenTheStringVariable) {
  const hldb::Begin *const body = getInitialBody();
  ASSERT_NE(body, nullptr);
  ASSERT_NE(body->getStmts(), nullptr);
  ASSERT_EQ(body->getStmts()->size(), 4u);
  ...
  for (uint32_t i = 0; i < 3u; ++i) {
    ...
    EXPECT_EQ(descriptor->getActual<hldb::Variable>(), getVariable("fd"))
        << "stmt[" << (i + 1u) << "]: Sec 21.3.2 puts the descriptor first";
    EXPECT_EQ(value->getActual<hldb::Variable>(), getVariable("str"))
        << "stmt[" << (i + 1u) << "]: all three calls write the same declared string variable";
  }

  for (uint32_t i = 0; i < 3u; ++i) {
    for (uint32_t j = i + 1u; j < 3u; ++j) {
      EXPECT_NE(calls[i], calls[j]) << "stmt[" << (i + 1u) << "] and stmt[" << (j + 1u)
...
TEST_F(FwriteBohTest, FinalBodyIsFcloseSysTaskCallOnTheSameFd) {
...
TEST_F(FwriteBohTest, CompilerReportsZeroErrors) {
