...
//   tests/Google/chapter-21/21.4--readmemh.sv   (:name: readmemh_task,
//   :description: $readmemh test, :tags: 21.4, :type: parsing)
// ----------------------------------------------------------------------------
//   module top();
//
...
//       "module top();" has an empty port list. Both "mem1" and "fname1" are
//       therefore module-level Variables and neither is a Net.
//   Sec 7.4.1 "Packed arrays" and Sec 7.4.2 "Unpacked arrays" -- "logic
//       [31:0] mem1 [1023:0];" writes dimensions on BOTH sides of the name,
...
//       the right way round is one of the two structural points of the
//       fixture: the object's own typespec is an array with the 1023:0 range,
//       and its ELEMENT type is the 32-bit logic vector.
//   Sec 6.16 "String data type" -- "string" is its own data type, so "fname1"
//       carries a StringTypespec; unlike "mem1" it is written with a
...
//       constant 8 bits wide per character, so "test1.mem" is 9 * 8 = 72
//       bits.
...
//       the file is named for. $readmemh is a system TASK, not a function:
...
//       The clause's two tasks differ in exactly one thing: the radix the
//       file's digits are read in -- binary for $readmemb, HEXADECIMAL for
//       $readmemh. Since a radix is applied while the file is being read, the
//       only half of that distinction which exists statically is the NAME, so
//       this file's other structural point is that the name must come back as
//       "$readmemh" and must not be its sibling. The companion fixture
//       21.4--readmemb.sv is identical but for that name and the objects'
//       suffixes, and the corresponding test asserts the mirror image.
//       Two further facts come straight from the source text rather than from
//       the clause in general:
//         * $readmemh opens, reads and closes the file itself. There is no
...
//         * the file name is passed as the IDENTIFIER "fname1", not as a
...
//           carrying "test1.mem" from the declaration.
//
// ----------------------------------------------------------------------------
// WHAT IS CHECKED (every assertion below names a concrete value):
//   - module "top" exists, has no Nets, no ports and no continuous
//     assignments, and declares exactly 2 Variables, "mem1" and "fname1",
//     which are distinct objects.
//   - "mem1"'s typespec is an ArrayTypespec whose getArrayType() is
...
//     bounds "31" and "0" -- the PACKED dimension written before it. "mem1"
...
//   - "fname1" resolves to a StringTypespec and NOT to an ArrayTypespec, and
//     carries a declaration initializer: a Constant with constType
//     vpiStringConst, value "test1.mem" and size 72 bits.
...
//     holds exactly ONE statement, which is not an Assignment -- $readmemh
//     needs no descriptor, so this fixture opens nothing and assigns nothing.
//   - that statement is a SysTaskCall named "$readmemh" -- and explicitly NOT
//     named "$readmemb", and explicitly NOT a SysFuncCall -- with exactly 2
//     arguments, which is what selects the Sec 21.4 two-argument form over
//     the start_addr and finish_addr forms.
//   - arg[0] is a RefObj named "fname1" bound by object identity to the
//     declared string Variable, and explicitly NOT a Constant; arg[1] is a
//     RefObj named "mem1" bound by object identity to the declared memory.
...
//   - That the contents of the file are interpreted as HEXADECIMAL digits,
//     which is the one thing separating $readmemh from $readmemb (Sec 21.4).
...
//     InitialBodyStatementIsReadmemhAndNotReadmemb.
//   - Whether "test1.mem" exists, whether it can be opened, and what data it
//     holds. Reading a file happens while time advances. The nearest real
//     assertion is StringVariableIsInitializedToTest1Mem, which pins the name
//     that would be opened.
//   - The values loaded into "mem1", and which of its 1024 locations get
//     written. Variable::getValue() exposes only a declaration-time
//     initializer, and "logic [31:0] mem1 [1023:0];" has none, so no field
//     anywhere records a post-load value. The nearest real assertion is
//     ReadmemhArgumentsAreFilenameThenMemory, which pins which declared
...
//     argument count of exactly 2 in ReadmemhArgumentsAreFilenameThenMemory,
//     which is what establishes that no address was supplied.
//   - Whether "fname1" still holds "test1.mem" by the time $readmemh reads
...
//     StringVariableIsInitializedToTest1Mem.
...
class ReadmemhTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "21.4--readmemh.hlc"}); }
 ...
 protected:
  ...
  // "$readmemh(fname1, mem1);" -- the block's one statement.
  static const hldb::SysTaskCall *getReadmemhCall() {
  ...
  static const hldb::ArrayTypespec *getMemArrayTypespec() {
    const hldb::Variable *const mem1 = getVariable("mem1");
    if (mem1 == nullptr || mem1->getTypespec<hldb::RefTypespec>() == nullptr) return nullptr;
    return mem1->getTypespec<hldb::RefTypespec>()->getActual<hldb::ArrayTypespec>();
  }
};

// --- module scope and its two declarations -----------------------------------

TEST_F(ReadmemhTest, ModuleTopExists) { EXPECT_NE(getTop(), nullptr); }
...
TEST_F(ReadmemhTest, ModuleHasNoNetsNoPortsAndNoContinuousAssignments) {
  const hldb::Module *const top = getTop();
...
TEST_F(ReadmemhTest, ModuleHasExactlyTwoDistinctVariablesMem1AndFname1) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  ...
  const hldb::Variable *const mem1 = getVariable("mem1");
  const hldb::Variable *const fname1 = getVariable("fname1");
  ASSERT_NE(mem1, nullptr);
  ASSERT_NE(fname1, nullptr);
  EXPECT_NE(mem1, fname1) << "the memory and the file name are two separate declarations and must be two "
                             "separate Variable objects";
}

// ONE OF THE TWO STRUCTURAL POINTS. Sec 7.4.1 / Sec 7.4.2: the dimension
// written AFTER the name is unpacked and belongs to the object, while the one
// written BEFORE it is packed and belongs to the element type. Asserting the
// bounds on each side separately is what proves the two halves were not
// swapped.
TEST_F(ReadmemhTest, Mem1IsAThousandTwentyFourDeepArrayOfThirtyTwoBitLogic) {
  const hldb::ArrayTypespec *const arr = getMemArrayTypespec();
  ASSERT_NE(arr, nullptr) << "Sec 7.4.2: 'mem1 [1023:0]' writes an unpacked dimension, so the object's own "
                             "type is an array";
  ...
  const hldb::Variable *const mem1 = getVariable("mem1");
  ASSERT_NE(mem1, nullptr);
  EXPECT_EQ(mem1->getValue(), nullptr) << "'logic [31:0] mem1 [1023:0];' is declared with no '=' initializer";
}
...
// not transposed.
TEST_F(ReadmemhTest, StringVariableIsInitializedToTest1Mem) {
  const hldb::Variable *const fname1 = getVariable("fname1");
  ASSERT_NE(fname1, nullptr);
  ASSERT_NE(fname1->getTypespec<hldb::RefTypespec>(), nullptr);

  EXPECT_NE(fname1->getTypespec<hldb::RefTypespec>()->getActual<hldb::StringTypespec>(), nullptr)
      << "Sec 6.16: 'string fname1' must resolve to a StringTypespec";
  EXPECT_EQ(fname1->getTypespec<hldb::RefTypespec>()->getActual<hldb::ArrayTypespec>(), nullptr)
      << "'fname1' is the string declaration, not the memory -- the two must not be transposed";

  const hldb::Constant *const init = fname1->getValue<hldb::Constant>();
  ASSERT_NE(init, nullptr) << "'string fname1 = \"test1.mem\";' carries a declaration-time initializer";
  EXPECT_EQ(init->getConstType(), vpiStringConst);
  EXPECT_EQ(init->getValue(), "test1.mem");
  EXPECT_EQ(init->getSize(), 72) << "Sec 5.9: \"test1.mem\" = 9 chars x 8 bits";
}

// --- the single procedure -----------------------------------------------------

// Sec 9.2.2: one "initial" keyword, one Initial process -- and nothing else.
TEST_F(ReadmemhTest, ModuleHasExactlyOneInitialProcess) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
// Sec 9.3.1 for the block, and Sec 21.4 for the consequence: $readmemh opens
...
TEST_F(ReadmemhTest, InitialBodyIsBeginWithOneNonAssignmentStatement) {
  const hldb::Begin *const body = getInitialBody();
  ...
  ASSERT_EQ(body->getStmts()->size(), 1u) << "the block holds the single '$readmemh(fname1, mem1);'";
  EXPECT_EQ(any_cast<hldb::Assignment>(body->getStmts()->at(0)), nullptr)
      << "Sec 21.4: $readmemh needs no file descriptor, so this fixture opens nothing and contains no "
         "assignment at all";
}

// --- the $readmemh call --------------------------------------------------------

// THE OTHER STRUCTURAL POINT. Sec 21.4 introduces $readmemh as a system TASK
// with no return value -- the opposite side of the task/function split from
// the Sec 21.3 routines, which all return an integer code -- and the NAME is
// the only statically observable thing that separates it from $readmemb,
// since the radix it implies is applied while the file is read.
TEST_F(ReadmemhTest, InitialBodyStatementIsReadmemhAndNotReadmemb) {
  const hldb::Begin *const body = getInitialBody();
  ...
  EXPECT_EQ(any_cast<hldb::SysFuncCall>(body->getStmts()->at(0)), nullptr)
      << "IEEE 1800-2023 Sec 21.4: $readmemh returns nothing, so it is a system task, not a system "
         "function";
  const hldb::SysTaskCall *const call = getReadmemhCall();
  ASSERT_NE(call, nullptr) << "the block's one statement should be a SysTaskCall";
  EXPECT_EQ(call->getName(), "$readmemh");
  EXPECT_NE(call->getName(), "$readmemb")
      << "Sec 21.4: the two tasks differ only in the radix they read the file's digits in, so the name "
         "is the whole of the distinction that exists before the file is read -- this fixture is the "
         "hexadecimal half, and 21.4--readmemb.sv is the binary one";
}

// Sec 21.4 fixes the order as (file_name, memory_name, ...). Exactly two
// arguments selects the shortest of the clause's three forms.
TEST_F(ReadmemhTest, ReadmemhArgumentsAreFilenameThenMemory) {
  const hldb::SysTaskCall *const call = getReadmemhCall();
  ASSERT_NE(call, nullptr);
  ASSERT_NE(call->getArguments(), nullptr);
  ASSERT_EQ(call->getArguments()->size(), 2u)
      << "'$readmemh(fname1, mem1)' passes the file name and the memory and nothing else, which is the "
...
TEST_F(ReadmemhTest, ReadmemhFilenameAndMemoryAreNotSwapped) {
  const hldb::SysTaskCall *const call = getReadmemhCall();
  ASSERT_NE(call, nullptr);
  ASSERT_NE(call->getArguments(), nullptr);
  ASSERT_EQ(call->getArguments()->size(), 2u);
...
TEST_F(ReadmemhTest, CompilerReportsZeroErrors) {
