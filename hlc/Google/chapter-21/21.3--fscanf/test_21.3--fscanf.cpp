...
//   tests/Google/chapter-21/21.3--fscanf.sv   (:name: fscanf_task,
//   :description: $fscanf test, :tags: 21.3, :type: simulation parsing)
// ----------------------------------------------------------------------------
//   module top();
//
//   int fd;
...
//       inside the block -- both variables are module-level here -- so the
//       Begin owns no variables of its own and every name used inside it
...
//       named for. The LRM's prototype is
//           integer code = $fscanf(fd, format, args);
//       so $fscanf is a system FUNCTION returning the number of items it
//       successfully matched. It is therefore a SysFuncCall even here, where
//       the file writes it in statement position and throws the count away --
//       discarding a result does not turn a system function into a system
//       task. The argument ORDER is the substance of this fixture and is the
//       exact opposite end of chapter 21's two conventions from its sibling
//       21.3--fread.sv: $fscanf puts the DESCRIPTOR FIRST, then the format
//       string, then the destination(s), whereas Sec 21.3.4.2's $fgets and
//       Sec 21.3.4.4's $fread put the destination first and the descriptor
//       second. A model that normalized every file-input call to one
//       argument order would place "fd" and "c" wrongly here, which is
//       precisely what the positional assertions below catch.
//   Sec 5.9 "String literals" -- a string literal is an unsigned integer
//       constant 8 bits wide per character: "tmp.txt" is 7 * 8 = 56 bits,
//       "w" is 1 * 8 = 8 bits and the format string "%d" is 2 * 8 = 16 bits.
//
// ----------------------------------------------------------------------------
...
//   - stmt[1] is a SysFuncCall named "$fscanf" and explicitly not a
//     SysTaskCall, with exactly 3 arguments, in this order: RefObj "fd"
//     bound to Variable "fd", a vpiStringConst Constant whose text is "%d"
//     and whose width is 16 bits, and RefObj "c" bound to Variable "c".
//   - within that call, the descriptor at arg[0] and the destination at
//     arg[2] resolve to two DIFFERENT Variable objects, and arg[0] is
//     specifically NOT the destination "c" -- the assertion that the
//     descriptor-first order of Sec 21.3.4.1 was not silently swapped for
//     $fread's destination-first order.
...
//   - What "%d" means when the call runs: that it consumes a decimal-radix
//     field from the stream and converts it. Interpreting a conversion
//     specification is formatting work done while time advances. What the
//     source DOES determine is the exact text and width of the format
//     string, and both are asserted by
//     FscanfArgumentsAreDescriptorFormatThenDestination.
//   - The value $fscanf parses into "c", and the match count it returns.
...
//     the same FscanfArgumentsAreDescriptorFormatThenDestination, which pins
//     which declared object the parsed value would be written into.
...
//   - That reading with $fscanf from a descriptor opened in "w" (write-only)
//     mode matches nothing. That is a runtime I/O outcome, not a
//     compile-time error -- Sec 21.3.1 makes the mode a runtime property of
//     the opened stream, and no LRM clause makes this combination illegal to
//     compile. The nearest real assertions are
//     FopenArgumentsAreFilenameAndMode (which pins the literal mode string
//     "w") and FscanfArgumentsAreDescriptorFormatThenDestination.
...
class FscanfTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "21.3--fscanf.hlc"}); }
 ...
 protected:
  ...
  // "$fscanf(fd, "%d", c);" -- stmt[1] of the initial block.
  static const hldb::SysFuncCall *getFscanfCall() {
...
TEST_F(FscanfTest, ModuleTopExists) { EXPECT_NE(getTop(), nullptr); }
...
TEST_F(FscanfTest, ModuleHasNoNetsNoPortsAndNoContinuousAssignments) {
  const hldb::Module *const top = getTop();
...
TEST_F(FscanfTest, ModuleHasExactlyTwoVariablesFdAndC) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
// initializer.
TEST_F(FscanfTest, VariablesFdAndCAreDistinctSignedInts) {
...
TEST_F(FscanfTest, ModuleHasInitialThenFinalProcess) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
...
TEST_F(FscanfTest, InitialBodyIsBeginWithTwoStatements) {
  const hldb::Begin *const body = getInitialBody();
  ...
  EXPECT_EQ(body->getStmts()->size(), 2u) << "'fd = $fopen(...);' and '$fscanf(fd, \"%d\", c);'";
}

// --- stmt[0]: fd = $fopen("tmp.txt", "w"); -----------------------------------

TEST_F(FscanfTest, FirstStatementAssignsFopenResultToFd) {
  const hldb::Begin *const body = getInitialBody();
...
TEST_F(FscanfTest, FopenArgumentsAreFilenameAndMode) {
  const hldb::Begin *const body = getInitialBody();
  ASSERT_NE(body, nullptr);
  const hldb::Assignment *const assign = any_cast<hldb::Assignment>(body->getStmts()->at(0));
...
// --- stmt[1]: $fscanf(fd, "%d", c); ------------------------------------------

// Sec 21.3.4.1: "integer code = $fscanf(fd, format, args);". The match count
// is discarded here by calling $fscanf in statement position, which does not
// make it a system task.
TEST_F(FscanfTest, SecondStatementIsFscanfSysFuncCall) {
  const hldb::Begin *const body = getInitialBody();
  ASSERT_NE(body, nullptr);
  EXPECT_EQ(any_cast<hldb::SysTaskCall>(body->getStmts()->at(1)), nullptr)
      << "IEEE 1800-2023 Sec 21.3.4.1: $fscanf returns the number of items matched, so it is a "
         "system function even when its result is discarded in statement position";
  const hldb::SysFuncCall *const fscanfCall = getFscanfCall();
  ASSERT_NE(fscanfCall, nullptr) << "stmt[1] should be a SysFuncCall";
  EXPECT_EQ(fscanfCall->getName(), "$fscanf");
}

// Sec 21.3.4.1 fixes the order as (fd, format, args): DESCRIPTOR FIRST, then
// the format string, then the destinations. Sec 5.9 fixes the format
// literal's width at 8 bits per character.
TEST_F(FscanfTest, FscanfArgumentsAreDescriptorFormatThenDestination) {
  const hldb::SysFuncCall *const fscanfCall = getFscanfCall();
  ASSERT_NE(fscanfCall, nullptr);
...
// destination-first order Sec 21.3.4.2 ($fgets) and Sec 21.3.4.4 ($fread)
// use. This pins that the two ends of the call were not normalized to one
// convention: arg[0] must be the descriptor and must NOT be the destination.
TEST_F(FscanfTest, FscanfDescriptorAndDestinationAreNotSwapped) {
  const hldb::SysFuncCall *const fscanfCall = getFscanfCall();
  ASSERT_NE(fscanfCall, nullptr);
  ASSERT_NE(fscanfCall->getArguments(), nullptr);
  ASSERT_EQ(fscanfCall->getArguments()->size(), 3u);
  ...
  EXPECT_EQ(first->getActual<hldb::Variable>(), getVariable("fd"))
      << "IEEE 1800-2023 Sec 21.3.4.1: $fscanf takes the descriptor FIRST, unlike $fgets and $fread";
  EXPECT_NE(first->getActual<hldb::Variable>(), getVariable("c"))
      << "arg[0] is the descriptor 'fd', not the destination 'c' -- the $fread argument order must "
         "not be applied here";
  EXPECT_NE(first->getActual<hldb::Variable>(), last->getActual<hldb::Variable>())
      << "the descriptor and the destination are distinct declared variables";
}
...
// contrast with the $fopen and $fscanf functions above.
TEST_F(FscanfTest, FinalBodyIsFcloseSysTaskCallOnTheSameFd) {
...
TEST_F(FscanfTest, CompilerReportsZeroErrors) {
