...
// than a constructor.
//
// FIXED COMPILER BUG #8 (super method-call resolution): "super.incs()" is
// parsed via the generic "Implicit_class_handle" ("super") + ordinary
// subroutine call path, and "super.new(...)" via a dedicated
// "Super_dot_new" grammar rule (Phase2ModelBuilder::leavePA_Super_dot_new)
// -- both lower to the same node type, MethodFuncCall. getTaskFunc() on
// "super.incs()" now correctly resolves to super_cls's base "incs", not
// test_cls's own override, matching what "super." is supposed to
// guarantee. TestIncsSecondStmtCallsSuperIncs below asserts this and now
// passes, alongside TestNewFirstStmtIsSuperNewCall's confirmation that
// "super.new(...)" resolves correctly too.
//
// Checked:
...
//     methods "incs" and "new") -- see the FIXED COMPILER BUG notes below
...
//     OWN "incs" Function per IEEE 8.15 (see FIXED COMPILER BUG #8 below)
//   - test_cls's "new": same shape as chapter-8/8.13--inheritance and
//     chapter-8/8.14--override_member's derived constructors --
//     "super.new(def + 3);" (a MethodFuncCall, same node type as
//     "super.incs()" and as an ordinary "new(...)" call) followed by
//     "a = def;"
...
// IEEE-mandated resolution and now passes. This was a DIFFERENT defect
// than FIXED COMPILER BUG #8 above: here getTaskFunc() used to resolve to
// null (never resolved at all); for BUG #8, getTaskFunc() resolved to a
// real Function, just the wrong one. Both are now fixed.
//
...
TEST_F(ClassSuperTest, ModuleHasTwoVariables) {
  const hldb::Module *const top = getTop();
...
// THE CRUX of this file, and FIXED COMPILER BUG #8:
// "incs = super.incs();" -- the lhs correctly resolves to test_cls's OWN
// "incs" (assign-to-own-name idiom, as usual), "super" correctly resolves
// to super_cls's ClassDefn, and the call itself (a MethodFuncCall, same
// node type "super.new(...)" uses) resolves getTaskFunc() to super_cls's
// OWN "incs", not test_cls's override -- exactly what "super." is
// supposed to guarantee. This EXPECT_EQ against getSuperIncsFunction()
// asserts the IEEE-mandated resolution and now passes; the paired
// EXPECT_NE against getTestIncsFunction() guards against a regression
// back to the self-referential override.
TEST_F(ClassSuperTest, TestIncsSecondStmtCallsSuperIncs) {
  ...
  ASSERT_NE(call, nullptr) << "'super.incs()' second path elem should be a MethodFuncCall (same node type as "
                              "'super.new(...)')";
...
TEST_F(ClassSuperTest, TestNewFirstStmtIsSuperNewCall) {
  ...
  EXPECT_EQ(superRef->getActual<hldb::ClassDefn>(), getSuperClsDefn());
  const hldb::MethodFuncCall *const call = any_cast<hldb::MethodFuncCall>(path->getPathElems()->at(1));
  ASSERT_NE(call, nullptr) << "'super.new(...)' second path elem should be a MethodFuncCall (same node type as "
                              "'super.incs()' above)";
...
TEST_F(ClassSuperTest, VariableTestObjExists) { EXPECT_NE(getVariableTestObj(), nullptr); }

TEST_F(ClassSuperTest, VariableTestObjTypespecResolvesToTestClsClassDefn) {
...
TEST_F(ClassSuperTest, VariableSuperObjExists) { EXPECT_NE(getVariableSuperObj(), nullptr); }

TEST_F(ClassSuperTest, VariableSuperObjTypespecResolvesToSuperClsClassDefn) {
...
TEST_F(ClassSuperTest, FirstStmtIsTestObjNewWithThirtySeven) {
  ...
  EXPECT_EQ(newCall->getTaskFunc(), getTestNewFunction())
      << "an ordinary 'new(...)' call should resolve getTaskFunc() to the user-written constructor (see FIXED "
