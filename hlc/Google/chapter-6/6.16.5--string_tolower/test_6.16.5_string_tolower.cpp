...
//   - variable 'b' initial value is a RefObj named "a.tolower()"
//   - RefObj element[0] is RefObj "a" with vpiActual resolving to Variable 'a'
//   - RefObj element[1] is FuncCall "tolower" with no arguments
...
// ---------------------------------------------------------------------------
// RefObj -- b's initial value is the method call a.tolower()
// ---------------------------------------------------------------------------
...
TEST_F(StringTolowerTest, BVariableValueIsNotPreEvaluatedConstant) {
  ...
  EXPECT_EQ(b->getValue<hldb::Constant>(), nullptr)
      << "HLC does not pre-evaluate a.tolower() to a constant; b holds only the RefObj expression";
}

TEST_F(StringTolowerTest, BVariableValueIsRefObj) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const b = hldb::findByName<hldb::Variable>("b", top->getVariables());
  ASSERT_NE(b, nullptr);
...
TEST_F(StringTolowerTest, RefObjReceiverIsA) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const b = hldb::findByName<hldb::Variable>("b", top->getVariables());
  ASSERT_NE(b, nullptr);
...
TEST_F(StringTolowerTest, RefObjMethodIsTolower) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const b = hldb::findByName<hldb::Variable>("b", top->getVariables());
  ASSERT_NE(b, nullptr);
  ...
  // IEEE 1800-2023 Sec 6.16: "tolower" is a string method. It is declared in no user scope,
  // so it can only resolve to the builtin "StringTypespec" class.
  const hldb::TaskFunc *const tf = call->getTaskFunc();
  ASSERT_NE(tf, nullptr) << "string.tolower() must bind (IEEE 1800-2023 Sec 6.16)";
  EXPECT_EQ(tf->getName(), "tolower");
...
TEST_F(StringTolowerTest, CompilerReportsZeroErrors) {
  ...
  EXPECT_EQ(findError(ErrorDefinition::LINT_NULL_ACTUAL), nullptr)
      << "str.tolower() must bind (IEEE 1800-2023 6.16.5)";
}
