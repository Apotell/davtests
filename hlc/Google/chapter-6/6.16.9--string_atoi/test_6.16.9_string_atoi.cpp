...
//   - variable 'b' initial value is a RefObj named "a.atoi()"
//   - RefObj element[0] is RefObj "a" with vpiActual resolving to Variable 'a'
//   - RefObj element[1] is FuncCall "atoi" with no arguments
...
TEST_F(StringAtoiTest, BVariableValueIsNotPreEvaluatedConstant) {
  ...
  EXPECT_EQ(b->getValue<hldb::Constant>(), nullptr)
      << "HLC does not pre-evaluate a.atoi() to a constant; b holds only the RefObj expression";
}

TEST_F(StringAtoiTest, BVariableValueIsRefObj) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const b = hldb::findByName<hldb::Variable>("b", top->getVariables());
  ASSERT_NE(b, nullptr);
...
TEST_F(StringAtoiTest, RefObjReceiverIsA) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const b = hldb::findByName<hldb::Variable>("b", top->getVariables());
  ASSERT_NE(b, nullptr);
...
TEST_F(StringAtoiTest, RefObjMethodIsAtoi) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const b = hldb::findByName<hldb::Variable>("b", top->getVariables());
  ASSERT_NE(b, nullptr);
  ...
  // IEEE 1800-2023 Sec 6.16: "atoi" is a string method. It is declared in no user scope,
  // so it can only resolve to the builtin "string_methods" class.
  const hldb::TaskFunc *const tf = call->getTaskFunc();
  ASSERT_NE(tf, nullptr) << "string.atoi() must bind (IEEE 1800-2023 Sec 6.16)";
  EXPECT_EQ(tf->getName(), "atoi");
