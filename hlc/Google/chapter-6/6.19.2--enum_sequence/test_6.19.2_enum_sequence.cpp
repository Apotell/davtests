...
// ranges", p.121): "name[N] ... Generates N named constants in the
// sequence: name0, name1, ..., nameN-1." "enum {start=10, step[10]} e;"
// is exactly this legal sequence syntax. No :should_fail_because: tag.
...
class EnumSequenceTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "6.19.2--enum_sequence.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

TEST_F(EnumSequenceTest, ModuleExists) {
  ASSERT_NE(hldb::findByName<hldb::Module>("top", m_design->getAllModules()), nullptr);
}

TEST_F(EnumSequenceTest, OneVariableExists) {
...
// ---------------------------------------------------------------------------
TEST_F(EnumSequenceTest, EVariableTypespecIsEnum) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(EnumSequenceTest, CompilerReportsZeroErrors) {
