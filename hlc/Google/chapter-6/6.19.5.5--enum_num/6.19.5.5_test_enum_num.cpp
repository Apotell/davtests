...
class EnumNum : public Test {
 public:
  static void SetUpTestSuite() {
    Compile(__FILE__, {"-f", "6.19.5.5--enum_num.hlc"});
...
TEST_F(EnumNum, NVariableIsIntType) {
  ...
  const uhdm::Begin *const blk = init->getStmt<uhdm::Begin>();
  ASSERT_NE(blk, nullptr);
  const uhdm::Variable *const n = blk->getVariables()->at(1);
  ASSERT_NE(n, nullptr);
  EXPECT_EQ(n->getName(), "n");
  EXPECT_NE(n->getTypespec()->getActual<uhdm::IntTypespec>(), nullptr)
...
TEST_F(EnumNum, NInitializerIsHierPath) {
  ...
  const uhdm::Variable *const n = blk->getVariables()->at(1);
  ASSERT_NE(n, nullptr);
  const uhdm::HierPath *const hp = n->getValue<uhdm::HierPath>();
  ASSERT_NE(hp, nullptr)
      << "n's vpiValue should be HierPath (inline initializer int n = val.num())";
