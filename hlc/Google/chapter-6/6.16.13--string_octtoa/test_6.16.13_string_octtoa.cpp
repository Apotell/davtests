...
//   - variable 'a' has no compile-time initial value (octtoa writes at runtime)
//   - top has 1 Initial process
//   - Initial stmt is a RefObj named "a.octtoa(12)"
//   - RefObj element[0] is RefObj "a" with vpiActual resolving to Variable 'a'
//   - RefObj element[1] is FuncCall "octtoa" with 1 argument (Constant "12")
...
class StringOcttoaTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "6.16.13--string_octtoa.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

TEST_F(StringOcttoaTest, ModuleExists) {
...
// ---------------------------------------------------------------------------
TEST_F(StringOcttoaTest, OneVariableExists) {
...
TEST_F(StringOcttoaTest, AVariableTypespecIsString) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(StringOcttoaTest, AVariableHasNoInitialValue) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const a = hldb::findByName<hldb::Variable>("a", top->getVariables());
  ASSERT_NE(a, nullptr);
...
// ---------------------------------------------------------------------------
TEST_F(StringOcttoaTest, InitialProcessExists) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
TEST_F(StringOcttoaTest, InitialStmtIsRefObj) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
...
// ---------------------------------------------------------------------------
// RefObj -- receiver 'a' and FuncCall 'octtoa' with 1 argument
// ---------------------------------------------------------------------------
TEST_F(StringOcttoaTest, RefObjReceiverIsA) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = dynamic_cast<const hldb::Initial *>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
...
TEST_F(StringOcttoaTest, RefObjMethodIsOcttoa) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = dynamic_cast<const hldb::Initial *>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
...
TEST_F(StringOcttoaTest, OcttoacArgumentIs12AsUIntConst) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = dynamic_cast<const hldb::Initial *>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
...
TEST_F(StringOcttoaTest, CompilerReportsZeroErrors) {
  ASSERT_NE(m_session->getErrorContainer(), nullptr);
  const ErrorContainer::Stats stats = m_session->getErrorContainer()->getErrorStats();
  EXPECT_EQ(stats.nbFatal, 0);
  EXPECT_EQ(stats.nbSyntax, 0);
  EXPECT_EQ(findError(ErrorDefinition::COMP_FAILED_TO_BIND, "octtoa"), nullptr)
      << "str.octtoa() must bind (IEEE 1800-2023 6.16.13)";
}
