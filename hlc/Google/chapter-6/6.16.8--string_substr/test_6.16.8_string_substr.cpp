...
TEST_F(StringSubstrTest, RefObjMethodIsSubstr) {
  ...
  // IEEE 1800-2023 Sec 6.16: "substr" is a string method. It is declared in no user scope,
  // so it can only resolve to the builtin "StringTypespec" class.
  const hldb::TaskFunc *const tf = call->getTaskFunc();
  ASSERT_NE(tf, nullptr) << "string.substr() must bind (IEEE 1800-2023 Sec 6.16)";
  EXPECT_EQ(tf->getName(), "substr");
