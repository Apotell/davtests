...
class LocalparamUnsignedIntTest : public Test {
 public:
  static void SetUpTestSuite() {
    Compile(__FILE__, {"-f", "6.20.4--localparam_unsigned_int.hlc"});
...
TEST_F(LocalparamUnsignedIntTest, ParameterCount) {
  ...
  EXPECT_EQ(m->getParameters()->size(), 1u)
      << "module 'top' declares exactly one localparam: q";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_Exists) {
  EXPECT_NE(getParam(m_design, "q"), nullptr)
      << "'q' not found in parameters";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_IsLocalParam) {
  ...
  EXPECT_TRUE(q->getLocalParam())
      << "ss.6.20.4: 'localparam int unsigned q' must be marked as a localparam";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_TypespecExists) {
  const hldb::Parameter *q = getParam(m_design, "q");
  ASSERT_NE(q, nullptr);
  EXPECT_NE(q->getTypespec(), nullptr)
      << "ss.6.11.2: 'localparam int unsigned q' must have a non-null typespec";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_Typespec_IsIntTypespec) {
  ...
  EXPECT_NE(rt->getActual<hldb::IntTypespec>(), nullptr)
      << "ss.6.11.2: 'int unsigned' must resolve to IntTypespec";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_Typespec_IsNotSigned) {
  ...
  EXPECT_FALSE(ts->getSigned())
      << "ss.6.11.2: 'int unsigned' must NOT be marked as signed";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_RhsIsConstant) {
  ...
  EXPECT_NE(pa->getRhs<hldb::Constant>(), nullptr)
      << "'localparam int unsigned q = 123': RHS must be a Constant";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_Rhs_ConstType_IsUnsignedInt) {
  ...
  EXPECT_EQ(c->getConstType(), vpiUIntConst)
      << "ss.6.11.2: 'int unsigned' value must have constType vpiUIntConst (9)";
}
...
TEST_F(LocalparamUnsignedIntTest, Q_RhsDecompile) {
  ...
  EXPECT_EQ(std::string(c->getDecompile()), "123")
      << "'localparam int unsigned q = 123': decompile must be \"123\"";
}
