...
class LocalparamTest : public Test {
 public:
  static void SetUpTestSuite() {
    Compile(__FILE__, {"-f", "6.20.4--localparam.hlc"});
...
TEST_F(LocalparamTest, ModuleExists) {
...
TEST_F(LocalparamTest, P_Exists) {
...
TEST_F(LocalparamTest, P_IsLocalParam) {
  ...
  EXPECT_TRUE(p->getLocalParam())
      << "ss.6.20.4: 'localparam p' must be marked as a localparam "
...
TEST_F(LocalparamTest, P_RhsIsConstant) {
  ...
  EXPECT_NE(pa->getRhs<hldb::Constant>(), nullptr)
      << "'localparam p = 123': RHS must be a Constant";
}
...
TEST_F(LocalparamTest, P_RhsDecompile) {
  ...
  EXPECT_EQ(std::string(c->getDecompile()), "123")
      << "'localparam p = 123': decompile must be \"123\"";
}
