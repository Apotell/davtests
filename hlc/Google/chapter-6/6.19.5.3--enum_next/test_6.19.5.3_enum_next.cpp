...
//   - assignment rhs is RefObj "val.next()"
//   - RefObj pathElems[0] is RefObj "val", pathElems[1] is FuncCall "next" (no args)
...
// ---------------------------------------------------------------------------
// Assignment: val = val.next() -- rhs is RefObj "val.next()"
// ---------------------------------------------------------------------------
TEST_F(EnumNextTest, AssignmentRhsIsRefObj) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = dynamic_cast<const hldb::Initial *>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
...
TEST_F(EnumNextTest, RefObjReceiverAndFuncCall) {
  ...
  EXPECT_EQ(call->getName(), std::string_view("next"));
  EXPECT_TRUE(call->getArguments() == nullptr || call->getArguments()->empty()) << "next() takes no arguments";
}

TEST_F(EnumNextTest, RefObjReceiverResolvesToVariable) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = dynamic_cast<const hldb::Initial *>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
...
TEST_F(EnumNextTest, NextCallBindsToBuiltinEnumerationIterator) {
  const hldb::Module *const top = hldb::findByName<hldb::Module>("top", m_design->getAllModules());
  ASSERT_NE(top, nullptr);
  const hldb::Initial *const init = dynamic_cast<const hldb::Initial *>(top->getProcesses()->at(0));
  ASSERT_NE(init, nullptr);
  ...
  const hldb::MethodFuncCall *const call = any_cast<hldb::MethodFuncCall>(hp->getPathElems()->at(1));
  ASSERT_NE(call, nullptr);
  // IEEE 1800-2023 Sec 6.19.5.3: "next()" is an enumerated-type method. It is declared in
  ...
  ASSERT_NE(tf, nullptr) << "enum.next() must bind (IEEE 1800-2023 Sec 6.19.5.3)";
  EXPECT_EQ(tf->getName(), "next");
...
TEST_F(EnumNextTest, CompilerReportsZeroErrors) {
  ...
  // reports LINT_NULL_ACTUAL instead, so check that (see Sec 6.19.5.3).
  EXPECT_EQ(findError(ErrorDefinition::LINT_NULL_ACTUAL), nullptr)
      << "enum.next() must bind (IEEE 1800-2023 Sec 6.19.5.3)";
}
