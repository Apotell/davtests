...
namespace hlc {
class MacroSvTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "macro_sv.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

// LRM 22.5.1: numeric macros must all be defined in macro.sv.
TEST_F(MacroSvTest, Number01MacroDefined) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ...
  ASSERT_NE(macro, nullptr) << "NUMBER_01 must be defined in macro.sv";
}

TEST_F(MacroSvTest, Number01MacroHasBody) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
...
// LRM 22.5.1: string macros must be defined.
TEST_F(MacroSvTest, String01MacroDefined) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
...
// LRM 22.5.1: MACRO_E is a SystemVerilog-only macro in macro.sv (absent from
// the iverilog-compatible macro_iv.sv). It must appear in the macro table.
TEST_F(MacroSvTest, MacroEDefined) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
      hldb::findByName<hldb::PreprocMacroDefinition>("MACRO_E", sf->getPreprocMacroDefinitions());
  ASSERT_NE(macro, nullptr) << "MACRO_E (SV-only) must be defined in macro.sv";
}

// LRM 22.5.1: MACRO_F must be defined.
TEST_F(MacroSvTest, MacroFDefined) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
      hldb::findByName<hldb::PreprocMacroDefinition>("MACRO_F", sf->getPreprocMacroDefinitions());
  ASSERT_NE(macro, nullptr) << "MACRO_F must be defined";
}

// LRM 22.5.1: the large multi-line MACRO_B must be defined.
TEST_F(MacroSvTest, MacroBDefined) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
...
TEST_F(MacroSvTest, Number01MacroObjectLike) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
...
TEST_F(MacroSvTest, MacroA01HasOneArg) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
...
// non-null.
TEST_F(MacroSvTest, MacroA01HasBody) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
...
// non-null.
TEST_F(MacroSvTest, MacroCArgCountAndBody) {
  ASSERT_NE(m_design->getSourceFiles(), nullptr);
  const hldb::SourceFile *const sf = hldb::findByName<hldb::SourceFile>("macro.sv", m_design->getSourceFiles());
  ASSERT_NE(sf, nullptr);
  const hldb::PreprocMacroDefinition *const macro =
