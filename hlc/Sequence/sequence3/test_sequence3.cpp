...
TEST_F(Sequence3, SequenceDeclaration) {
  ...
  for (const uhdm::SequenceDecl *const s : *tb->getSequenceDecls()) {
    if (s->getName() == "seq3") { seq3 = s; break; }
  }
...
TEST_F(Sequence3, SequenceExprIsConsecutiveRepeat) {
  ...
  EXPECT_EQ(expr->getOpType(), vpiConsecutiveRepeatOp)
      << "seq3 op is not vpiConsecutiveRepeatOp (expected a[*2])";
}
...
TEST_F(Sequence3, SequenceExprHasTwoOperands) {
  ...
  EXPECT_EQ(expr->getOperands()->size(), 2u)
      << "expected 2 operands (signal 'a' and repetition count 2)";
}
...
TEST_F(Sequence3, AssertPropertyExprReferencesSeq3) {
  ...
  EXPECT_EQ(propExpr->getName(), "seq3")
      << "inline assert property expression does not reference 'seq3'";
}
