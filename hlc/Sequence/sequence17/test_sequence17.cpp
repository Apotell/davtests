...
//   Assert_PropertyExpr_ResolvedToSeqNonconsecDecl -- 'seq_nonconsec' in
//                         assert property(@(posedge clk) seq_nonconsec) is
//                         treated as an implicit net (EL0535) instead of
...
TEST_F(Sequence17Test, Net_clk_HasBitTypespec) {
...
TEST_F(Sequence17Test, Net_a_HasBitTypespec) {
  const hldb::Module *const tb = getTb(m_design);
  ASSERT_NE(tb, nullptr);
  ASSERT_NE(tb->getNets(), nullptr);
...
TEST_F(Sequence17Test, SeqDecl_seq_nonconsec_Op_Operand0_ResolvesToNet) {
  ...
  EXPECT_NE(ref->getActual<hldb::Net>(), nullptr)
      << "RefObj 'a' in non-consecutive repeat must resolve to the Net "
         "declaration";
}
...
TEST_F(Sequence17Test, Assert_PropertyExpr_ResolvedToSeqNonconsecDecl) {
  ...
  // the SequenceDecl. Surelog bug EL0535 treats it as an implicit net
  // instead; getActual<SequenceDecl>() returns null. This test FAILS
  ...
  EXPECT_NE(propExpr->getActual<hldb::SequenceDecl>(), nullptr)
      << "EL0535: 'seq_nonconsec' in assert property must resolve to the "
