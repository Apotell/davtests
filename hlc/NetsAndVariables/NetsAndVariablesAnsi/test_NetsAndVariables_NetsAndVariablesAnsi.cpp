...
//   - nets_and_variables_test exists with ports clk, a, b (input) and y (output)
...
class NetsAndVariablesAnsi : public Test {
 ...
 protected:
  ...
  static const hldb::Module *getSecond() {
    return hldb::findByName<hldb::Module>("nets_and_variables_second", m_design->getAllModules());
  }
};
...
TEST_F(NetsAndVariablesAnsi, ModportInterfaceExists) {
  EXPECT_NE(hldb::findByName<hldb::Interface>("nets_and_variables_modport_if", m_design->getAllInterfaces()),
            nullptr);
}

TEST_F(NetsAndVariablesAnsi, ModportInterfaceHasNetAndVariable) {
  const hldb::Interface *const iface =
      hldb::findByName<hldb::Interface>("nets_and_variables_modport_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
...
TEST_F(NetsAndVariablesAnsi, ClassExists) {
  ...
  EXPECT_NE(hldb::findByName<hldb::ClassDefn>("nets_and_variables_class", m_design->getAllClasses()), nullptr);
}
...
TEST_F(NetsAndVariablesAnsi, AssertionModuleExists) {
  EXPECT_NE(hldb::findByName<hldb::Module>("nets_and_variables_assertion", m_design->getAllModules()), nullptr);
}

TEST_F(NetsAndVariablesAnsi, AssertionModuleHasThreeInputPorts) {
  const hldb::Module *const mod =
      hldb::findByName<hldb::Module>("nets_and_variables_assertion", m_design->getAllModules());
