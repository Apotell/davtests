...
//     LHS of a continuous assignment) and implicit_var_a/b/c (undeclared
//     identifiers used as the LHS of a procedural assignment) are NOT
//     materialized as a Net or Variable, matching the documented behavior in
//     6.10--implicit_continuous_assignment (HLC reports EL0535 "Illegal
//     implicit net" and does not add a graph node for the identifier)
...
// ---------------------------------------------------------------------------
TEST_F(NetsAndVariablesAnsi, ImplicitNetANotDeclared) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  ASSERT_NE(top->getNets(), nullptr);
...
TEST_F(NetsAndVariablesAnsi, ImplicitNetBNotDeclared) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  ASSERT_NE(top->getNets(), nullptr);
...
TEST_F(NetsAndVariablesAnsi, SecondContAssignDrivesImplicitNetAWithNoActual) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  ASSERT_NE(top->getContAssigns(), nullptr);
...
// = 7 total.
// ---------------------------------------------------------------------------
TEST_F(NetsAndVariablesAnsi, SevenProcessesExist) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  ASSERT_NE(top->getProcesses(), nullptr);
  EXPECT_EQ(top->getProcesses()->size(), 7u);
}
...
TEST_F(NetsAndVariablesAnsi, ProcessTypeCounts) {
  ...
  EXPECT_EQ(alwaysCount, 6) << "always_comb x2, always_ff x2, always_latch, always";
}
...
// ---------------------------------------------------------------------------
// Class -- no test anywhere in this suite demonstrates reaching a file-scope
// (non-nested) hldb::ClassDefn from the Design; left as a documented
// placeholder rather than a guessed API call.
// ---------------------------------------------------------------------------
TEST_F(NetsAndVariablesAnsi, ClassDeclarationNotStructurallyChecked) {
  GTEST_SKIP() << "No test in this suite demonstrates looking up a file-scope hldb::ClassDefn from the Design "
                  ...
                  "nets_and_variables_class is left unchecked rather than guessing an unverified API.";
}
