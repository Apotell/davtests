...
//
// None of these declarations use a net-type keyword (wire/tri/...), so per
// IEEE 1800 Annex A.2.1.3 they all go through data_declaration, never
// net_declaration -- every one of them must materialize as a hldb::Variable
// in getVariables(), and must NOT also appear in getNets() (no duplicate).
//
// Checked:
//   - each identifier is a hldb::Variable, absent from getNets()
//   - typespec assertions wherever an existing test in this suite already
//     establishes the pattern (int -> IntTypespec, real -> RealTypespec,
//     string -> StringTypespec, event -> EventTypespec, enum ->
...
TEST_F(AnsiVariablesTest, VarRegIsLogicTypespec) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_reg");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarIntegerIsIntegerTypespec) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_integer");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarShortintExists) {
  ...
  // suite, so only existence (and no Net duplicate) is checked.
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  EXPECT_NE(getVarNoNetDuplicate(top, "var_shortint"), nullptr);
}

TEST_F(AnsiVariablesTest, VarLongintIsLongIntTypespec) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_longint");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarTimeIsTimeTypespec) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_time");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarRealIsRealTypespec) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_real");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarRealtimeTypespecActualIsNull) {
  ...
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_realtime");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarStringIsStringTypespec) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_string");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarChandleTypespecActualIsNull) {
  ...
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_chandle");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
  ASSERT_NE(rts->getActual(), nullptr);
...
TEST_F(AnsiVariablesTest, VarEnumIsEnumTypespec) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_enum");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
...
TEST_F(AnsiVariablesTest, VarRegVectorIsSevenToZero) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Variable *const v = getVarNoNetDuplicate(top, "var_reg_vector");
  ASSERT_NE(v, nullptr);
  const hldb::RefTypespec *const rts = v->getTypespec();
  ASSERT_NE(rts, nullptr);
  const hldb::LogicTypespec *const ls = rts->getActual<hldb::LogicTypespec>();
