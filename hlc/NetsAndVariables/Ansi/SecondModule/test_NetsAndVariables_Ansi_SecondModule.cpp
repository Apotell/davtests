...
//
// Checked:
//   - nets_and_variables_second exists
//   - it instantiates the interface (ref instance "if0")
//   - it declares a class-handle variable ("cls0"), which must not also
//     appear in getNets() (no duplicate) -- a class handle is never subject
//     to the net/variable ambiguity ports and net_declaration face
...
TEST_F(AnsiSecondModuleTest, SecondModuleHasCls0VariableNoNetDuplicate) {
  const hldb::Module *const second = getSecond();
  ASSERT_NE(second, nullptr);
  ...
  EXPECT_EQ(hldb::findByName<hldb::Net>("cls0", second->getNets()), nullptr)
      << "'cls0' is a class-handle variable -- it must not also appear in vpiNet";
}
