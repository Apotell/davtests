...
// Validates the UHDM graph produced for tests/NetsAndVariables/Ansi/Package.sv,
// split out of the combined NetsAndVariablesAnsi.sv suite so the file-scope
// package testing point stands on its own.
//
// Checked:
//   - nets_and_variables_pkg is reachable via Design::getAllPackages()
...
// it -- see ContAssignIsIllegalInsidePackage below.
...
class AnsiPackageTest : public Test {
 public:
 ...
 protected:
  static const hldb::Package *getPkg() {
    return hldb::findByName<hldb::Package>("nets_and_variables_pkg", m_design->getAllPackages());
  }
};

TEST_F(AnsiPackageTest, PackageExists) {
  ASSERT_NE(m_design->getAllPackages(), nullptr);
  ASSERT_NE(getPkg(), nullptr);
}

TEST_F(AnsiPackageTest, PkgLogicIsVariableNoNetDuplicate) {
...
TEST_F(AnsiPackageTest, PkgRegIsVariableNoNetDuplicate) {
  const hldb::Package *const pkg = getPkg();
  ASSERT_NE(pkg, nullptr);
...
TEST_F(AnsiPackageTest, PkgWireIsNetWithWireTypeNoVariableDuplicate) {
  const hldb::Package *const pkg = getPkg();
  ASSERT_NE(pkg, nullptr);
...
TEST_F(AnsiPackageTest, ContAssignIsIllegalInsidePackage) {
