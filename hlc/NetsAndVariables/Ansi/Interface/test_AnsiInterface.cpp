...
TEST_F(AnsiInterfaceTest, InterfaceHasOneContAssign) {
  const hldb::Interface *const iface =
      hldb::findByName<hldb::Interface>("work@nets_and_variables_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
...
TEST_F(AnsiInterfaceTest, ContAssignDrivesImplicitNetWire) {
  const hldb::Interface *const iface =
      hldb::findByName<hldb::Interface>("work@nets_and_variables_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
  ASSERT_NE(iface->getContAssigns(), nullptr);
...
TEST_F(AnsiInterfaceTest, ImplicitNetIsDeclaredWire) {
  const hldb::Interface *const iface =
      hldb::findByName<hldb::Interface>("work@nets_and_variables_if", m_design->getAllInterfaces());
  ASSERT_NE(iface, nullptr);
  ASSERT_NE(iface->getNets(), nullptr);
  const hldb::Net *const net = hldb::findByName<hldb::Net>("if_implicit_net", iface->getNets());
  ASSERT_NE(net, nullptr) << "'if_implicit_net' is a legally-implicit wire net and should be materialized";
