...
TEST_F(AnsiImplicitTest, ExplicitLogicIsFormallyDeclaredVariable) {
  const hldb::Module *const top = getTop();
  ASSERT_NE(top, nullptr);
  const hldb::Net *const l = hldb::findByName<hldb::Net>("explicit_logic", top->getNets());
  ASSERT_NE(l, nullptr);
...
TEST_F(AnsiImplicitTest, ImplicitWireIsDeclaredWire) {
  ...
  ASSERT_NE(w, nullptr) << "'implicit_wire' is a legally-implicit wire net (IEEE 1800 clause 6.10) and "
...
TEST_F(AnsiImplicitTest, ImplicitNetAIsDeclaredWire) {
  ...
  ASSERT_NE(net, nullptr) << "'implicit_net_a' is a legally-implicit wire net (IEEE 1800 clause 6.10) and "
                             "should be materialized";
...
TEST_F(AnsiImplicitTest, ImplicitNetBIsDeclaredWire) {
  ...
  ASSERT_NE(net, nullptr) << "'implicit_net_b' is a legally-implicit wire net (IEEE 1800 clause 6.10) and "
                             "should be materialized";
  EXPECT_EQ(net->getNetType(), vpiWire);
}
