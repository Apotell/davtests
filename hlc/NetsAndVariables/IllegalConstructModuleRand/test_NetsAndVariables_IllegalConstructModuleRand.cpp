...
// Tests for tests/NetsAndVariables/illegal_construct/Illegal_construct_module_rand.sv
//
...
class IllegalConstructModuleRandTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "IllegalConstructModuleRand.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }
};

TEST_F(IllegalConstructModuleRandTest, RandVariableAtModuleScopeIsRejected) {
  ASSERT_NE(m_session->getErrorContainer(), nullptr);
  const ErrorContainer::Stats stats = m_session->getErrorContainer()->getErrorStats();
  EXPECT_GT(stats.nbFatal + stats.nbSyntax + stats.nbError, 0)
      << "a `rand` qualifier on a variable declared directly inside a module is illegal: rand/randc "
