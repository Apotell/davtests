...
//     continuous assignment) is NOT materialized as a Net, matching
//     6.10--implicit_continuous_assignment
...
// a continuous assignment and was never declared. Matches
// 6.10--implicit_continuous_assignment: no Net node is materialized.
// ---------------------------------------------------------------------------
TEST_F(NetsAndVariablesNonAnsi, ImplicitNetNonansiNotDeclared) {
  const hldb::Module *const mod = getMod();
  ASSERT_NE(mod, nullptr);
...
TEST_F(NetsAndVariablesNonAnsi, ImplicitNetNonansiContAssignHasNoActual) {
  const hldb::Module *const mod = getMod();
  ASSERT_NE(mod, nullptr);
...
// ---------------------------------------------------------------------------
// Class -- left unchecked; see the ANSI test file for rationale
// ---------------------------------------------------------------------------
TEST_F(NetsAndVariablesNonAnsi, ClassDeclarationNotStructurallyChecked) {
