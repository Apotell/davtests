...
//
// Checked:
//   - work@nets_and_variables_program exists with three ports (a, b input;
//     y output)
...
class NonAnsiProgramTest : public Test {
 ...
 protected:
  static const hldb::Program *getProg() {
    return hldb::findByName<hldb::Program>("work@nets_and_variables_program", m_design->getAllPrograms());
  }
};

TEST_F(NonAnsiProgramTest, ProgramExists) { ASSERT_NE(getProg(), nullptr); }

TEST_F(NonAnsiProgramTest, ProgramHasThreePorts) {
...
TEST_F(NonAnsiProgramTest, ProgramHasNetAndVariable) {
  const hldb::Program *const prog = getProg();
  ASSERT_NE(prog, nullptr);
...
TEST_F(NonAnsiProgramTest, ProgramHasOneContAssignDrivingProgNet) {
  const hldb::Program *const prog = getProg();
  ASSERT_NE(prog, nullptr);
...
TEST_F(NonAnsiProgramTest, ProgramHasOneProcess) {
  const hldb::Program *const prog = getProg();
  ASSERT_NE(prog, nullptr);
