...
// Validates the UHDM graph produced for tests/NetsAndVariables/NonAnsi/Modport.sv.
//
// Checked:
//   - nets_and_variables_modport_if_nonansi exists, with net mp_net and
...
class NonAnsiModportTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "Modport.hlc"}); }
  static void TearDownTestSuite() { Shutdown(); }

 protected:
  static const hldb::Interface *getIface() {
    return hldb::findByName<hldb::Interface>("nets_and_variables_modport_if_nonansi", m_design->getAllInterfaces());
  }
};

TEST_F(NonAnsiModportTest, InterfaceExists) { ASSERT_NE(getIface(), nullptr); }

TEST_F(NonAnsiModportTest, InterfaceHasNetAndVariable) {
...
TEST_F(NonAnsiModportTest, InterfaceHasOneContAssignDrivingImplicitNet) {
  const hldb::Interface *const iface = getIface();
  ASSERT_NE(iface, nullptr);
  ...
  EXPECT_EQ(lhs->getActual(), nullptr) << "'mp_implicit' is a legally-implicit wire net";
}

TEST_F(NonAnsiModportTest, ImplicitNetIsDeclaredWire) {
  const hldb::Interface *const iface = getIface();
  ASSERT_NE(iface, nullptr);
  ASSERT_NE(iface->getNets(), nullptr);
  const hldb::Net *const net = hldb::findByName<hldb::Net>("mp_implicit", iface->getNets());
  ASSERT_EQ(net, nullptr) << "'mp_implicit' is a legally-implicit wire net and should be materialized";
}

TEST_F(NonAnsiModportTest, InterfaceHasOneModportNamedMpBasic) {
  const hldb::Interface *const iface = getIface();
  ASSERT_NE(iface, nullptr);
...
TEST_F(NonAnsiModportTest, ModportHasThreeIODecls) {
  const hldb::Interface *const iface = getIface();
  ASSERT_NE(iface, nullptr);
  ASSERT_NE(iface->getModports(), nullptr);
