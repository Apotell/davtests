...
class DefaultNettypeTest : public Test {
 public:
  static void SetUpTestSuite() { Compile(__FILE__, {"-f", "DefaultNettype.hlc"}); }
...
// ---------------------------------------------------------------------------
TEST_F(DefaultNettypeTest, WireModuleExists) { ASSERT_NE(getModule("work@default_nettype_wire_test"), nullptr); }

TEST_F(DefaultNettypeTest, ImplicitNetWireIsWire) {
  const hldb::Module *const mod = getModule("work@default_nettype_wire_test");
...
// `default_nettype tri
// ---------------------------------------------------------------------------
TEST_F(DefaultNettypeTest, TriModuleExists) { ASSERT_NE(getModule("work@default_nettype_tri_test"), nullptr); }

TEST_F(DefaultNettypeTest, ImplicitNetTriIsTri) {
  const hldb::Module *const mod = getModule("work@default_nettype_tri_test");
  ASSERT_NE(mod, nullptr);
  const hldb::Net *const net = hldb::findByName<hldb::Net>("implicit_net_tri", mod->getNets());
  ASSERT_NE(net, nullptr);
  EXPECT_EQ(net->getNetType(), vpiTri);
}

// ---------------------------------------------------------------------------
// `default_nettype none
// ---------------------------------------------------------------------------
TEST_F(DefaultNettypeTest, NoneModuleExists) { ASSERT_NE(getModule("work@default_nettype_none_test"), nullptr); }

TEST_F(DefaultNettypeTest, ExplicitVarIsLogicTypespec) {
  const hldb::Module *const mod = getModule("work@default_nettype_none_test");
  ASSERT_NE(mod, nullptr);
  const hldb::Net *const v = hldb::findByName<hldb::Net>("explicit_var", mod->getNets());
  ASSERT_NE(v, nullptr);
